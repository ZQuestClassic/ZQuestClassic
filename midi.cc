...
bool decode_text_event(char *s,byte type,byte *buf)
{
  switch(type)
  {
  case 0x01: sprintf(s,"T: %s\n",buf); break;
  case 0x02: sprintf(s,"C: %s\n",buf); break;
  case 0x03: sprintf(s,"N: %s\n",buf); break;
  case 0x04: sprintf(s,"I: %s\n",buf); break;
  default:
    return false;
  }
  return true;
...
// must have called get_midi_info() first
{
  ...
  for(int i=0; midi->track[i].len>0; i++)
  {
    ...
    while(!eot(mi) && data - midi->track[i].data < midi->track[i].len)
    {
      ...
      if(mi->event==0xFF)
      {
        memcpy(buf,mi->buf,mi->nbytes);
        buf[mi->nbytes]=0;
        if(decode_text_event(out,mi->type,buf))
          out+=strlen(out);
      }
    }
  }
