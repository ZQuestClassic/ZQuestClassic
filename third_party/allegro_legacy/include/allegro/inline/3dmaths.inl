...
AL_LEGACY_INLINE(fixed, dot_product, (fixed x1, fixed y_1, fixed z1, fixed x2, fixed y2, fixed z2),
{
   return fixmul(x1, x2) + fixmul(y_1, y2) + fixmul(z1, z2);
})


AL_LEGACY_INLINE(float, dot_product_f, (float x1, float y_1, float z1, float x2, float y2, float z2),
{
   return (x1 * x2) + (y_1 * y2) + (z1 * z2);
})


AL_LEGACY_INLINE(void, persp_project, (fixed x, fixed y, fixed z, fixed *xout, fixed *yout),
{
...
AL_LEGACY_INLINE(void, persp_project_f, (float x, float y, float z, float *xout, float *yout),
{
   float z1 = 1.0f / z;
