...
When a `WaitX` command is hit, the compiled function saves where to resume and returns; the next time it is called it picks up at the instruction after that `WaitX`. The native backends track resume points via `JittedScript::pc_to_resume_address`. x64 jumps straight to the saved native address through an annotated indirect branch. a64 picks its dispatch form per function: with few resume points (<= 32) it compares `ctx->pc` against each one on entry (direct branches, so the register allocator can put fixup moves on each edge), and past that it uses the same indirect branch as x64. The comparison chain runs on *every* entry, so it must stay short: a function with thousands of call sites (decompiled 2.50-era quests link everything into one giant global-script function) re-walked the chain on every return from a callee, which made the JIT up to 30x slower than the interpreter - and the slowdown grew the deeper into the function the game's active call sites sat, since their resume points sit later in the chain. (An earlier ~10% penalty for the indirect form no longer reproduces; `-jit-a64-annotated-resume` forces it everywhere for A/B.) On the wasm backend the resume point is a saved block index (`wait_index`) that the function's loop-switch dispatches on when re-entered.

### Direct calls between compiled functions (native backends)

The driver loop in `jit_shared.cpp` normally orchestrates every script function call: a `CALLFUNC` exits the caller's compiled code back to C++, which looks up the callee (a binary search over function start pcs), pushes the ZASM return pc, and re-enters the callee's compiled code through its prologue; every `RETURNFUNC` does the mirror image, plus a resume-address lookup to re-enter the caller mid-function. That round-trip exists because a script can `WaitX` mid-call-stack: native stack frames can't survive returning to C++, so call state has to live in the ZASM return stack instead.

But a function that can never yield (no `WaitX` transitively reachable - the same `may_yield` analysis the wasm backend uses to split its module) never needs to suspend, so calls to it are emitted as plain native calls on the machine stack. The call site loads the callee's entry from the per-script `direct_entry_table` (0 until the callee is compiled and committed, in which case it falls back to the driver path - the slot doubling as the hot-swap point), passes the return pc in `ctx->call_pc`, sets `ctx->entry_mode = 1`, and calls. The callee's entry pushes the ZASM return stack via `jit_direct_enter` (so the driver's view of the call chain stays exact) and skips its resume dispatch; its `RETURNFUNC` pops the return stack and returns `EXEC_RESULT_RETURN` to the call site instead of to the driver.

Everything unusual is handled by one rule: a direct call site treats any result other than `EXEC_RESULT_RETURN` as an unwind, and returns it unchanged from its own function. A `QUIT`, an error from an interpreter batch, or a driver-path fallback deep in a chain of direct calls just ripples up the native frames to the driver, which handles it exactly as if it had made the calls itself - the ZASM return stack and `ctx->pc`/`ctx->sp` were maintained identically along the way. (`ctx->pc` mirrors the driver's call/return updates deliberately: error stack traces are built from it.) Two guard rails: past 256 ZASM call frames, `jit_direct_enter` converts the call into a driver-path call, since native frames are real stack memory (`stack_limit.zs`'s 10000-deep recursion exercises this); and on x64 the call site preserves the single-slot `ctx->saved_regs` pinned-register save area around the nested call, which nested prologues would otherwise clobber.

This removes the dominant cost for call-dense scripts - std.zh-heavy quests make thousands of tiny helper calls per frame, sometimes averaging only a dozen ZASM instructions of real work per call. Across the `measure_performance.py` benchmark set, total script time dropped ~30%, with the most call-heavy replays down 60%+. The tradeoff is compile time: each direct call site embeds a native call, which asmjit's register allocation charges for superlinearly in huge functions (a 206k-instruction 2.50-era global script went from ~1s to ~6s to compile, on the precompile workers). Disable with `-no-jit-direct-calls`.

A function with too many direct call sites keeps the driver call path for all of them (`jit_direct_calls_worth_it`, default cap 2000, `-jit-direct-calls-max-sites`). Past a few thousand sites the InvokeNodes make compiles pathological (the 149k-instruction decompiled 2.50 global script of "The Adventure of Link and Zelda: Panoply of Calatia" - replay upload `69162522CA4C5DAD4DA0CAEDA505FBFB.qst` - has ~12.7k call sites: 44s -> 206s), and - worse - each site's call-crossing virtual registers get their own spill slots, so the frame grows ~32 bytes per site; asmjit emits no stack probes, so a frame that large skips the Windows guard page on entry and crashes no matter the stack size. Only ~10 of the ~400k functions across the replay-test + uploaded-quest corpus exceed the cap, and gating them was measured to cost under 1% of the direct-call win (a gated function's hot callees still direct-call each other; huge many-site functions are cold dispatchers).
...
- **Non-yielding functions** compile to individual wasm functions with real structured control flow (`WasmStructurer` in `wasm_structurer.h`, a dominator-tree-guided translation). Calls to them are plain wasm calls; the native call stack handles returns. (The native backends now apply the same idea to non-yielding callees - see "Direct calls between compiled functions" above.) Irreducible control flow falls back to the loop-switch lowering below.
