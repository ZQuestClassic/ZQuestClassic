...
Zelda Classic requires a modified version of the Allegro 4.4.4 library. Again, pre-built library binaries are available in `/libs`, and if you need to rebuild the library from source, it can be found on GitHub at https://github.com/ArmageddonGames/allegro5/tree/4.4-ZCFixes. The patches that you need to apply are in the 4.4-ZCFixes branch of this repository (**not** master or 4.4).

A stripped-down version of the fixed library, containing only the header files needed to compile ZC, is in `./allegro`. **Do not try to link Zelda Classic against the standard 4.4.4 Allegro library. You must use the pre-built binaries, or the modified source.** Otherwise you will experience some hardware issues, such as loss of keyboard input while playing.
