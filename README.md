...
Zelda Classic requires a modified version of the Allegro 4.4.2 library. Again, pre-built library binaries are available in `/libs`, and if you need to rebuild the library from source, it is contained in `./allegro/fixed/allegro-4.4.2.zip`. The patches that you need to apply are in `other/allegro_patch.` Replace the files from the archive with the corresponding files in the patch directory, otherwise ZC will experience problems (such as crashes in fullscreen mode on Windows.)

A stripped-down version of the fixed library, containing only the header files needed to compile ZC, is in `./allegro`. **Do not try to link Zelda Classic against the standard 4.4.2 Allegro library. You must use the pre-built binaries, or the modified source.**
