...
{
    switch(getGuyProp(id, GUYPROP_FAMILY))
    {
    case eeROCK:
    case eeTEK:
        return true;
    ...
    case eeWALK:
        if(getGuyProp(id, GUYPROP_MISC9)==e9tVIRE || getGuyProp(id, GUYPROP_MISC9)==e9tPOLSVOICE)
            return true;
    }
    ...
    case eeGUY:
        {
            ...
            switch(getGuyProp(id, GUYPROP_MISC10))
            {
            case 1:
            case 2:
                return true;
...
bool flyerblocked(int dx, int dy, int special)
{
    return ((special==spw_floater)&&
            ((COMBOTYPE(dx,dy)==cNOFLYZONE)||
             (getComboClassProp(COMBOTYPE(dx,dy), CCPROP_BLOCK_ENEMIES)&4)!=0||
...
int GuyHit(int tx,int ty,int tz,int txsz,int tysz,int tzsz)
{
    ...
    {
        enemy@ en=getGuy(i);
        if(en.hit(tx,ty,tz,txsz,tysz,tzsz))
        {
            if(en.stunclk==0 && (!questRuleEnabled(qr_SAFEENEMYFADE) || en.fading != fade_flicker)
                    &&(en.family != eeGUY || en.dmisc1!=0))
            {
                return i;
            }
        }
