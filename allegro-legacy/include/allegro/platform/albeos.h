...
AL_LEGACY_VAR(SYSTEM_DRIVER, system_beos);

/* timer driver */
#define TIMER_BEOS            AL_ID('B','T','I','M')
AL_LEGACY_VAR(TIMER_DRIVER, timer_beos);

/* keyboard driver */
#define KEYBOARD_BEOS         AL_ID('B','K','E','Y')
AL_LEGACY_VAR(KEYBOARD_DRIVER, keyboard_beos);

/* mouse driver */
#define MOUSE_BEOS            AL_ID('B','M','O','U')
AL_LEGACY_VAR(MOUSE_DRIVER, mouse_beos);

/* joystick driver */
#define JOYSTICK_BEOS         AL_ID('B','J','O','Y')
AL_LEGACY_VAR(JOYSTICK_DRIVER, joystick_beos);

/* graphics drivers */
#define GFX_BWINDOWSCREEN_ACCEL  AL_ID('B','W','S','A')
#define GFX_BWINDOWSCREEN        AL_ID('B','W','S',' ')
AL_LEGACY_VAR(GFX_DRIVER, gfx_beos_bwindowscreen_accel);
AL_LEGACY_VAR(GFX_DRIVER, gfx_beos_bwindowscreen);

#define GFX_BDIRECTWINDOW        AL_ID('B','D','W','N')
#define GFX_BWINDOW              AL_ID('B','W','N',' ')
AL_LEGACY_VAR(GFX_DRIVER, gfx_beos_bdirectwindow);
AL_LEGACY_VAR(GFX_DRIVER, gfx_beos_bwindow);

#define GFX_BWINDOW_OVERLAY      AL_ID('B','O','V','L')
AL_LEGACY_VAR(GFX_DRIVER, gfx_beos_overlay);

/* digi drivers */
#define DIGI_BEOS             AL_ID('B','D','I','G')
AL_LEGACY_VAR(DIGI_DRIVER, digi_beos);

/* midi drivers */
#define MIDI_BEOS             AL_ID('B','M','I','D')
AL_LEGACY_VAR(MIDI_DRIVER, midi_beos);
