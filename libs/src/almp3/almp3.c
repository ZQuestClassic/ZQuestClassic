...
#include <mpg123.h>
#include <mpglib.h>
#include <stdint.h> //zero 07-feb-2016
...
ALMP3_MP3 *almp3_create_mp3(void *data, int data_len) {
  ...
  (uint8_t*)data += data_start_skipped; //zero 07-feb-2016
  ...
  /* test the next 16 * MAXFRAMESIZE bytes after ID3v2 until we find one valid frame */
  {
    ...
    while (!found && (skipbytes < max)) {
      ...
      unsigned char *p = (uint8_t*)data + skipbytes; //zero 07-feb-2016
    ...
    data_len -= skipbytes;
    (uint8_t*)data += skipbytes; //zero 07-feb-2016
  }
