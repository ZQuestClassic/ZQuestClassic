...
void StunGuy(int j,int stun)
{
    ...
    if(((enemy*)guys.spr(j))->z==0 && canfall(((enemy*)guys.spr(j))->id))
    {
        ...
        ((enemy*)guys.spr(j))->fall=-zc_min(FEATHERJUMP,(stun*8)+zc_rand()%5);
    }
}
...
//Confuse the cheaters by moving the game data to a random location
    ...
    char *dummy = (char *) zc_malloc((zc_rand()%(RAND_MAX/2))+32);
    game = new gamedata;
    game->Clear();
...
void do_magic_casting()
{
    ...
    case itype_faroreswind:
    {
        ...
        {
            Link.setX(tempx+((zc_rand()%3)-1));
            Link.setY(tempy+((zc_rand()%3)-1));
        }
...
void game_loop()
{
    ...
    //  putpixel(framebuf, walkflagx, walkflagy+playing_field_offset, vc(int(zc_rand()%16)));
}
...
void runDrunkRNG(){
	...
	for(int i = 0; i<sizeof(drunk_toggle_state); i++){
		if((!(frame%((zc_rand()%100)+1)))&&(zc_rand()%MAXDRUNKCLOCK<Link.DrunkClock())){
			drunk_toggle_state[i] = (zc_rand()%2)?true:false;
		} else {
