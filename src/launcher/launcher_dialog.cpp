...
TextField(fitParent = true, \
	...
	bounds = {int32_t(_min*int32_t(pow(10, _places))), int32_t(_max*int32_t(pow(10, _places)))}, \
...
TextField(fitParent = true, \
	...
	bounds = {_min, _max}, val = zc_get_config(head, subhead, def, app), \
	onValChangedFunc = [&](GUI::TextField::type,std::string_view,int32_t val) \
	{ \
		zc_set_config(head, subhead, val, app); \
	}), \
			...
			tabPanel = TabPanel(
				...
				TabRef(name = "ZC Player", Row(framed = true,
					Rows<2>(fitParent = true,
						...
						CONFIG_CHECKBOX_I("Native File Dialog",App::zelda,"gui","native_file_dialog",1,"Use the file dialog provided by the OS for file select prompts."),
				...
				TabRef(name = "ZC Editor", Row(framed = true,
					Rows<2>(fitParent = true,
						...
						CONFIG_CHECKBOX_I("Native File Dialog",App::zquest,"gui","native_file_dialog",1,"Use the file dialog provided by the OS for file select prompts."),
