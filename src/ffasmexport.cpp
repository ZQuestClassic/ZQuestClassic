		...
		if(variable_list[q].maxcount>0)
		{
			...
			if(arg >= start && arg < start+(variable_list[q].maxcount*mult))
			{
				for(int w = 0; w < variable_list[q].maxcount; ++w)
				{
					...
					char buf[64];
					if(strcmp(variable_list[q].name, "A"))
...
string getOpcodeString(ffscript const* line)
{
	...
	if(s_c.args == 2)
	{
		if(s_c.arg1_type == 0)
		{
			...
			{
				if(line->arg2 % 10000 != 0) sprintf(a2buf, "%ld.%04d", line->arg2 / 10000, line->arg2 % 10000);
		...
		else
		{
			if(s_c.arg2_type == 0)
			{
				if(line->arg1 % 10000 != 0) sprintf(a1buf, "%ld.%04d", line->arg1 / 10000, line->arg1 % 10000);
