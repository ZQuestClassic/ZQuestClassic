...
void bmp_do_rectr(BITMAP *bmp, int *sdci, int xoffset, int yoffset)
{
    ...
    if(sdci[10]==0) //no rotation
    {
        if(sdci[11]) //filled
        {
            rectfill(bitty, x1+xoffset, y1+yoffset, x2+xoffset, y2+yoffset, color);
        }
        else //outline
        {
            rect(bitty, x1+xoffset, y1+yoffset, x2+xoffset, y2+yoffset, color);
        }
    }
    else  //rotate
    {
        ...
        if(sdci[11]) //filled
        {
            polygon(bitty, 4, xy, color);
        }
        else //outline
        {
            line(bitty, xy[0], xy[1], xy[10], xy[11], color);
            line(bitty, xy[2], xy[3], xy[12], xy[13], color);
            line(bitty, xy[4], xy[5], xy[14], xy[15], color);
            line(bitty, xy[6], xy[7], xy[ 8], xy[ 9], color);
        }
    }
...
void bmp_do_circler(BITMAP *bmp, int *sdci, int xoffset, int yoffset)
{
    ...
    if(sdci[10]) //filled
    {
        circlefill(bitty, x1+xoffset, y1+yoffset, r, color);
    }
    else //outline
    {
        circle(bitty, x1+xoffset, y1+yoffset, r, color);
    }
...
void bmp_do_arcr(BITMAP *bmp, int *sdci, int xoffset, int yoffset)
{
    ...
    if(sdci[12]) //closed
    {
        if(sdci[13]) //filled
        {
            ...
            {
                draw_trans_sprite(bitty, prim_bmp, 0,0);
            }
            else
            {
                draw_sprite(bitty, prim_bmp, 0,0);
            }
        }
        else
        {
            arc(bitty, cx+xoffset, cy+yoffset, sa, ea, int(r), color);
            line(bitty, cx+xoffset, cy+yoffset, cx+xoffset+fixtoi(fixcos(-sa)*r), cy+yoffset+fixtoi(fixsin(-sa)*r), color);
            line(bitty, cx+xoffset, cy+yoffset, cx+xoffset+fixtoi(fixcos(-ea)*r), cy+yoffset+fixtoi(fixsin(-ea)*r), color);
        }
    }
    else
    {
...
void bmp_do_drawlayerr(BITMAP *bmp, int *sdci, int xoffset, int yoffset, bool isOffScreen)
{
    //sdci[1]=layer
    //sdci[2]=map
    //sdci[3]=screen
...
void bmp_do_drawscreenr(BITMAP *bmp, int *sdci, int xoffset, int yoffset, bool isOffScreen)
{
    //sdci[1]=layer
    //sdci[2]=map
    //sdci[3]=screen
...
void bmp_do_primitives(BITMAP *targetBitmap, int type, mapscr *, int xoff, int yoff)
{
    ...
    for(int i(0); i < numDrawCommandsToProcess; ++i)
    {
        ...
        case BMPRECTR:
        {
            bmp_do_rectr(bmp, sdci, xoffset, yoffset);
        }
        break;
        ...
        case BMPCIRCLER:
        {
            bmp_do_circler(bmp, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPARCR:
        {
            bmp_do_arcr(bmp, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPELLIPSER:
        {
            bmp_do_ellipser(bmp, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPLINER:
        {
            bmp_do_liner(bmp, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPSPLINER:
        {
            bmp_do_spliner(bmp, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPPUTPIXELR:
        {
            bmp_do_putpixelr(bmp, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPDRAWTILER:
        {
            bmp_do_drawtiler(bmp, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPDRAWCOMBOR:
        {
            bmp_do_drawcombor(bmp, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPFASTTILER:
        {
            bmp_do_fasttiler(bmp, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPFASTCOMBOR:
        {
            bmp_do_fastcombor(bmp, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPDRAWCHARR:
        {
            bmp_do_drawcharr(bmp, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPDRAWINTR:
        {
            bmp_do_drawintr(bmp, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPDRAWSTRINGR:
        {
            bmp_do_drawstringr(bmp, i, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPQUADR:
        {
            bmp_do_drawquadr(bmp, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPQUAD3DR:
        {
            bmp_do_drawquad3dr(bmp, i, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPTRIANGLER:
        {
            bmp_do_drawtriangler(bmp, sdci, xoffset, yoffset);
        }
        break;
        
        case BMPTRIANGLE3DR:
        {
            bmp_do_drawtriangle3dr(bmp, i, sdci, xoffset, yoffset);
        }
        break;
	/*
	case BMPPOLYGONR:
        {
            bmp_do_polygonr(bmp, i, sdci, xoffset, yoffset);
        }
        break;
	*/
        ...
        case BMPDRAWLAYERR:
        {
           bmp_do_drawlayerr(bmp, sdci, xoffset, yoffset, isTargetOffScreenBmp);
        }
        break;
        
        case BMPDRAWSCREENR:
        {
            bmp_do_drawscreenr(bmp, sdci, xoffset, yoffset, isTargetOffScreenBmp);
        }
        break;
