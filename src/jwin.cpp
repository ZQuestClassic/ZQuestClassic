...
{
	GUI::ListData *data = (GUI::ListData *)d->dp;
	char *sel = (char *)d->dp2;
	...
	height = (d->h-3) / text_height(*data->ListFont);
	
	i = MID(0, ((gui_mouse_y() - d->y - 4) / text_height(*data->ListFont)),
			((d->h-3) / text_height(*data->ListFont) - 1));
	i += d->d2;
	...
	if(i != d->d1)
	{
		...
		_handle_jwin_scrollable_scroll(d, listsize, &d->d1, &d->d2, *data->ListFont);
...
void _jwin_draw_abclistbox(DIALOG *d)
{
	...
	for(i=0; i<height; i++)
	{
		if(d->d2+i < listsize)
		{
			...
			strncpy(s, data->getText(i+d->d2).c_str(), 1023);
			x = d->x + 4;
			y = d->y + 4 + i*text_height(*data->ListFont);
			//		 text_mode(bg);
			rectfill(screen, x, y, x+7, y+text_height(*data->ListFont)-1, bg);
			x += 8;
			len = (int32_t)strlen(s);
			
			while(text_length(*data->ListFont, s) >= d->w - (bar ? 26 : 10))
			{
				len--;
				s[len] = 0;
			}
			
			textout_ex(screen, *data->ListFont, s, x, y, fg,bg);
			x += text_length(*data->ListFont, s);
			
			if(x <= d->x+w)
				rectfill(screen, x, y, d->x+w, y+text_height(*data->ListFont)-1, bg);
		}
		else
			rectfill(screen, d->x+2,  d->y+4+i*text_height(*data->ListFont),
					 d->x+w+2, d->y+3+(i+1)*text_height(*data->ListFont), bg_color);
	}
...
int32_t jwin_list_proc(int32_t msg, DIALOG *d, int32_t c)
{
	...
	case MSG_CLICK:
		...
		{
			_handle_jwin_scrollable_scroll_click(d, listsize, &d->d2, *data->ListFont);
		}
		break;
	...
	case MSG_WHEEL:
		height = (d->h-4) / text_height(*data->ListFont);
	...
	case MSG_CHAR:
		if(listsize)
		{
			...
			bottom = d->d2 + (d->h-3)/text_height(*data->ListFont) - 1;
...
static int32_t droplist(DIALOG *d)
{
	...
	y = d->y + d->h;
	h = zc_min(abc_patternmatch ? listsize+1 : listsize,8) * text_height(*data->ListFont) + 8;
	...
	{
		w=zc_min(max_w,zc_max(w,text_length(*data->ListFont,data->getText(i).c_str())+39));
	}
...
int32_t jwin_abclist_proc(int32_t msg,DIALOG *d,int32_t c)
{
	...
	if(abc_patternmatch) // Search style pattern match. 
	{
		if(msg==MSG_CHAR && ((c&0xFF) > 31) && ((c&0xFF) < 127)) //(isalpha(c&0xFF) || isdigit(c&0xFF)))
		{
			...
			h = ((d->h-3) / text_height(*data->ListFont))-1;
			...
			for ( int32_t listpos = 0; listpos < max; ++listpos )
			{
				...
				strcpy(lsttmp, ((data->getText(listpos).c_str())));
	...
	else // Windows Explorer style jumping
	{
		if(msg==MSG_CHAR && (isalpha(c&0xFF) || isdigit(c&0xFF)))
		{
			...
			h = (d->h-3) / text_height(*data->ListFont);
			c = toupper(c&0xFF);
			...
			for(i=cur+1; (cur ? (i != cur) : (cur < max)); ++i) //don't infinite loop this. 
			{
				...
				if(i>=max) i=0;
				if(toupper((data->getText(i).c_str())[0]) == c)
				{
					d->d1 = i;
