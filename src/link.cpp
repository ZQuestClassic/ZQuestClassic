...
void LinkClass::resetflags(bool all)
{
    ...
    inwallm=inceilingm=infloorm=false;
...
{
    if(!(inwallm||inceilingm||infloorm) && (action==none || action==walking))
    {
...
void LinkClass::init()
{
    ...
    id=0;
    inlikelike=0;
    superman=inwallm=inceilingm=infloorm=false;
    scriptcoldet=1;
    ...
    if(action != casting)
    {
        ...
        case las_original:                                               //normal
            ...
            else if(action==swimming || action==swimhit || hopclk==0xFF)
            {
                ...
                (&tile, &flip, &extend, is_moving()?ls_swim:ls_float, dir, zinit.linkanimationstyle);
...
{
    if(infloorm) //ugly hack... but it works.
    {
        set_clip_rect(dest,x+xofs,y+yofs,x+xofs+(txsz*16-1),y+yofs+(tysz*16-1));
        set_clip_state(dest,1);
    }

    if(isdungeon() && currscr<128 && (x<16 || x>224 || y<18 || y>146) && !get_bit(quest_rules,qr_FREEFORM) && !(inceilingm||infloorm))
    {
        // clip under doorways
...
void LinkClass::checkhit()
{
    ...
    if(hclk>0 || inlikelike == 1 || action==inwind || action==drowning || (inwallm||inceilingm||infloorm) || diveclk>30 || (action==hopping && hopclk<255))
    {
        return;
    }
...
//printf("Stomp check: %d <= 12, %d < %d\n", int((y+16)-(((enemy*)guys.spr(hit2))->y)), (int)falling_oldy, (int)y);
    ...
    case eeWALK:
    {
        ...
        if(dm7 >= e7tEATITEMS && dm7 <= e7tEATHURT)
        {
            EatLink(hit2);
...
bool LinkClass::animate(int)
{
    ...
    if(!inceilingm) //ceilingmasters obstruct gravity.
    {
    ...
    if(inwallm||inceilingm||infloorm)
    {
        attackclk=0;
        linkstep();
...
void LinkClass::checklocked()
{
	...
	*/
	...
	bool found=false; //crash bug fixed -Tamamo
	for ( int q = 0; q < 4; q++ ) {
