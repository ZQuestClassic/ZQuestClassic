...
static void InitMatrix()
{
    ...
    {
        column[i].speed = ((zc_rand() % (_speed + 8)) * (7 - _speed)) + (7 - _speed) * 6;
        column[i].cnt = 0;
        activecol[i] = 0;
    }
...
static void AddTracer()
{
    ...
    if(--delay <= 0)
    {
        delay = zc_rand() % (32 - _density*3 - _speed*2);
        
        for(int i=0; i<maxtracers; i++)
        {
            if(tracer[i].speed == 0)
            {
                tracer[i].x = zc_rand()%cols;
                tracer[i].y = 0;
                tracer[i].speed = (zc_rand()&7) * (4-_speed/2) + 8 - _speed;
                tracer[i].cnt = 0;
...
static void AddEraser(int col = -1)
{
    for(int i=0; i<maxtracers; i++)
    {
        if(eraser[i].speed == 0 && (col>=0 || (tracer[i].speed
                                               ...
                                               && (zc_rand() % ((7 + _density - _speed)*5 + 10)) == 0)))
        {
            eraser[i].x = col>=0 ? col : tracer[i].x;
            eraser[i].y = 0;
            eraser[i].speed = col>=0 ? (zc_rand()&7) * (4-_speed/2) + 8 - _speed : tracer[i].speed + (zc_rand()&3);
            eraser[i].cnt = 0;
