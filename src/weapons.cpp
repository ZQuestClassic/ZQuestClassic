        ...
        if(id<wEnemyWeapons)
        {
            if(clk==(misc-1))
            {
                ...
                for(int tx=-f1; tx<=f1; tx+=8)  // -16,-8,0,8,16
                {
                    ...
                    {
                        findentrance(x+tx,y+ty+(isSideview()?2:-3),mfBOMB,true);

                        if(id==wSBomb || id==wLitSBomb)
                        {
                            findentrance(x+tx,y+ty+(isSideview()?2:-3),mfSBOMB,true);
                        }

                        findentrance(x+tx,y+ty+(isSideview()?2:-3),mfSTRIKE,true);
                    }
                }
            }
