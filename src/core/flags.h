...
constexpr uint32_t F(int n)
{
	return 1 << n;
}

constexpr uint64_t LLF(int n)
{
	return 1ULL<<n;
}
...
enum weapon_flags : uint16_t
{
	...
	WFLAG_TEMP_IGNORE_SOLID         = F(11),
	WFLAG_OBEY_CONVEYORS            = F(12),
	WFLAG_CARRYABLE_CROSS_SCREEN    = F(13),
	WFLAG_MAX                       = 14,
	WFLAG_BURNFLAGS                 = (WFLAG_BURN_ANYFIRE|WFLAG_BURN_STRONGFIRE|WFLAG_BURN_MAGICFIRE|WFLAG_BURN_DIVINEFIRE)
};
