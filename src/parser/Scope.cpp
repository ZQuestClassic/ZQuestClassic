...
static Datum* _lookupDatum(Scope& scope, std::string const& name, ASTExprIdentifier& host, CompileErrorHandler* errorHandler, bool forceSkipUsing, bool trimScriptLocal, bool trimClassLocal, bool* trimmed_match)
{
	...
	for (; current; current = current->getParent())
	{
		...
		{
			if (current->getClassScope()->getClassVar(name))
				*trimmed_match = true;
		}
...
bool BasicScope::add(Datum& datum, CompileErrorHandler* errorHandler)
{
	...
	// Script-scope datums are never stack-backed: nonstatic variables live in
	// the script's instance data, statics are globals, and constants are
	// inlined. (The instance var register is assigned after this point, in
	// register_instance_var, so it cannot be checked here.)
	if (!ZScript::isGlobal(datum) && !isScript())
	{
