...
bool read_source_007(const char* srcfile, bool packed, const char* password, std::vector<byte>& raw)
{
	...
	if (packed)
	{
		PACKFILE* src = pack_fopen_password(srcfile, F_READ_PACKED, password);
		if (!src && errno == EDOM)
			src = pack_fopen_password(srcfile, F_READ, password);
		if (!src)
			return false;
		byte buf[65536];
		while (true)
...
PACKFILE* open_decoded_stream_007(const char* srcfile, const char* header, int32_t method, bool packed, const char* password)
{
	...
	if (packed)
	{
		s->src = pack_fopen_password(srcfile, F_READ_PACKED, password);
		if (!s->src && errno == EDOM)
			s->src = pack_fopen_password(srcfile, F_READ, password);
	}
	else
	{
		s->src = pack_fopen_password(srcfile, F_READ, "");
		if (s->src)
		{
