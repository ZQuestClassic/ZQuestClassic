			...
			switch(tempitem.family)
			{
				...
				case itype_divinefire:
					if(!(tempitem.flags & item_flag3))
						break;
					[[fallthrough]];
				case itype_candle: case itype_book: case itype_bait:
					tempitem.weap_data.moveflags |= move_obeys_grav | move_can_pitfall;
					break;
			}
		}
...
int32_t readcombo_loop(PACKFILE* f, word s_version, newcombo& temp_combo)
{
	...
	if(combo_has_flags)
	{
		...
		if(combo_has_flags&CHAS_LIFT)
		{
			...
			auto& weap_data = temp_combo.lift_weap_data.get();
			...
			temp_combo.lift_weap_data.normalize();
		}
		else temp_combo.lift_weap_data.reset();
		...
		{
			if(auto ret = read_weap_data(temp_combo.misc_weap_data.get(), f))
				return ret;
			temp_combo.misc_weap_data.normalize();
		}
	}
