...
{
	// Engine fixes
	set_qr(qr_DMGCOMBOLAYERFIX, 1);
	set_qr(qr_WARPSIGNOREARRIVALPOINT, 1);
	set_qr(qr_NOARRIVALPOINT, 1);
	...
	set_qr(qr_CUSTOMCOMBOSLAYERS1AND2, 1);
	set_qr(qr_CUSTOMCOMBOS_EVERY_LAYER, 1);
	set_qr(qr_HOOKSHOTLAYERFIX, 1);
	set_qr(qr_EPILEPSY, 1);
	set_qr(qr_FIXSCRIPTSDURINGSCROLLING, 1);
	set_qr(qr_ALWAYS_DEALLOCATE_ARRAYS, 1);
	set_qr(qr_ITEMSCRIPTSKEEPRUNNING, 1);
	set_qr(qr_FIXDRUNKINPUTS, 1);
	set_qr(qr_BITMAP_AND_FILESYSTEM_PATHS_ALWAYS_RELATIVE, 1);
	...
	set_qr(qr_LOG, 1);
	set_qr(qr_TRACESCRIPTIDS, 1);
	// Misc Engine stuff (not really related to any particular ruleset, so, always on)
	set_qr(qr_PASSIVE_SUBSCRIPT_RUNS_DURING_ACTIVE_SUBSCRIPT, 1);
	set_qr(qr_PASSIVE_SUBSCRIPT_RUNS_WHEN_GAME_IS_FROZEN, 1);
	set_qr(qr_COMBOSCRIPTS_LAYER_0, 1);
	set_qr(qr_ONDEATH_RUNS_AFTER_DEATH_ANIM, 1);
	set_qr(qr_FFCSCROLL, 1);
	set_qr(qr_GREATER_MAX_TIME, 1);
	/* currently bugged: wBrang Lv2 and above. --I think we fixed this? -Z 8th Feb, 2020 */
	set_qr(qr_WEAPONS_EXTRA_DEATH_FRAME, 1);
	set_qr(qr_USE_EX1_EX2_INVENTORYSWAP, 1);
	set_qr(qr_CLEARINITDONSCRIPTCHANGE, 1);
	set_qr(qr_SCRIPT_WEAPONS_UNIQUE_SPRITES, 1); //Give Script 1 to 10 weapons, generated by ZScript unique sprites.
	set_qr(qr_WRITING_NPC_WEAPON_UNIQUE_SPRITES, 1);
	set_qr(qr_WRITE_ENTRYPOINTS_AFFECTS_HEROCLASS, 1);
	set_qr(qr_SMARTER_SMART_SCROLL, 1);
	set_qr(qr_PASSIVE_ITEM_SCRIPT_ONLY_HIGHEST, 1);
	set_qr(qr_QUAKE_STUNS_LEEVERS, 1);
	set_qr(qr_CAN_PLACE_GANON, 1);
	set_qr(qr_CAN_PLACE_TRAPS, 1);
	set_qr(qr_PERMANENT_WHISTLE_SECRETS, 1);
	set_qr(qr_STEP_IS_FLOAT, 1);
}
...
void applyRuleset(int32_t newRuleset, byte *qrptr)
{
	...
	static const int rBSZ[] = {
		...
		qr_FADE, qr_EXPANDEDLTM, qr_OLDSPRITEDRAWS, qr_UNBEATABLES_DONT_KEEP_DEAD, qr_NO_LEAVE_ONE_ENEMY_ALIVE_TRICK,
	};
	...
	static const int rMODERN[] = {
		...
		qr_EW_FIRE_EMITS_LIGHT, qr_RESPAWN_POINTS_HALF_GRID,
		qr_KEEPOLD_APPLIES_RETROACTIVELY,
		qr_SCC_ITEM_COMBINES_ITEMS,
		qr_SCC_GOTO_RESPECTS_CONTFLAG, qr_FIND_CLOSEST_BAIT,
		qr_SMARTDROPS, qr_SMARTER_DROPS, qr_SMARTDROPS_NOTHING,
		qr_GRASS_SENSITIVE, qr_BETTER_RAFT,
		qr_BETTER_RAFT_2, qr_RAFT_SOUND, qr_POUNDLAYERS1AND2,
		qr_HELD_BOMBS_EXPLODE, qr_MIRROR_PRISM_LAYERS,
		qr_SEPARATE_BOMBABLE_TAPPING_SFX,
	};
	static const int rMODERN_O[] = {
		qr_OLDSPRITEDRAWS, qr_OLD_RESPAWN_POINTS,
