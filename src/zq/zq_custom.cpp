...
void edit_enemydata(int32_t index)
{
	...
	for(int32_t i=0; i<32; i++)
		enedata_dlg[74+i].flags = (guysbuf[index].flags & LLF(i)) ? D_SELECTED : 0;
		
	enedata_dlg[186].d1 = (guysbuf[index].flags & guy_fade_instant ? 2
						   : guysbuf[index].flags & guy_fade_flicker ? 1 : 0);
						   
	for(int32_t i=0; i<16; i++)
		enedata_dlg[106+i].flags = (guysbuf[index].flags & LLF(i+32)) ? D_SELECTED : 0;
	
	for(int32_t i=0; i<16; i++)
		enedata_dlg[399+i].flags = (guysbuf[index].flags & LLF(i+32+16)) ? D_SELECTED : 0;
	...
	do
	{
		...
		for(int32_t i=0; i<32; i++)
			test.flags |= (guy_flags)((enedata_dlg[74+i].flags & D_SELECTED) ? LLF(i) : 0);
		...
		for(int32_t i=0; i<16; i++)
			test.flags |= (guy_flags)((enedata_dlg[106+i].flags & D_SELECTED) ? LLF(i+32) : 0);
		
		for(int32_t i=0; i<16; i++)
			test.flags |= (guy_flags)((enedata_dlg[399+i].flags & D_SELECTED) ? LLF(i+32+16) : 0);
