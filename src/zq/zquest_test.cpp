...
// itemdata has no operator==, but the loader memsets each entry before
// filling it, so a byte compare is sound.
template<typename T>
...
// wpndata has no operator== either, and the loader copies it in from a
// stack local without zeroing it first, so its padding byte holds whatever
// was on the stack. Compare the fields, not the bytes.
bool wpns_equal(std::vector<wpndata> const& a, std::vector<wpndata> const& b)
{
	if (a.size() != b.size())
		return false;
	for (size_t i = 0; i < a.size(); ++i)
	{
		auto const& x = a[i];
		auto const& y = b[i];
		if (x.tile != y.tile || x.misc != y.misc || x.csets != y.csets
			|| x.frames != y.frames || x.speed != y.speed || x.type != y.type
			|| x.script != y.script)
			return false;
	}
	return true;
...
void test_defaults_apply()
{
	...
	assertTrue(!wpns_equal(after.sprites, before.sprites));
	...
	assertTrue(wpns_equal(after.sprites, expected.sprites));
}
