...
const std::vector<int> group_editor_ids = {
	...
	ZQKEY_DMAPS, ZQKEY_MAP_STYLES, ZQKEY_STRINGS, ZQKEY_ENDSTRING,
	ZQKEY_EDIT_ENEMIES, ZQKEY_EDIT_ITEMS, ZQKEY_EDIT_DROPSETS, ZQKEY_EDIT_SPRITE_DATA,
	ZQKEY_EDIT_SUBSCREENS,
	ZQKEY_PLAYER_SPRITE, ZQKEY_GAME_ICONS, ZQKEY_MCGUFFIN_PIECES, ZQKEY_SHOP_TYPES,
	ZQKEY_INFO_TYPES,
...
class BackdropRTI : public RenderTreeItem
{
public:
	BackdropRTI(std::string name) : RenderTreeItem(name) {};
...
BackdropRTI rti_hotkeys_backdrop("hot_keys_backdrop");
...
private:
	...
	void prepare()
	{
		...
		set_transform({
			.x = (int)(root->width - w*scale) / 2,
			.y = (int)(root->height - h*scale) / 2,
			.xscale = scale,
			.yscale = scale,
		});
