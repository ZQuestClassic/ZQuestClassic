	...
	while(!scriptChoice)
	{
		...
		switch(indx)
		{
			case 1: //confirm; exit dlg
			{
				...
				//}
				//{ Find export file
				if(!prompt_for_new_file_compat("Export Script (.zasm)","zasm",zasm_extlist,datapath,false))
				{
					scriptChoice = NULL;
					break;
				}
...
//  Frameskip					 = zc_get_config("zquest","frameskip",0); //todo: this is not actually supported yet.
	...
	if (quick_assign_arg > 0)
	{
		...
		success = save_quest(argv[quick_assign_arg + 1]) == 0;
		if (!success)
		{
	...
	if (smart_assign_arg > 0)
	{
		...
		success = save_quest(argv[smart_assign_arg + 1]) == 0;
		if (!success)
		{
			printf("Failed to save quest\n");
			exit(1);
