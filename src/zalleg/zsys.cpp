...
int32_t decode_file_007(const char *srcfile, const char *destfile, const char *header, int32_t method, bool packed, const char *password)
{
    ...
    // open files
    size = zalleg_file_size_ex_password(srcfile, password);
    ...
    {
        packed_src = zalleg_pack_fopen_password(srcfile, F_READ_PACKED,password);
        
        if(errno==EDOM)
        {
            packed_src = zalleg_pack_fopen_password(srcfile, F_READ,password);
        }
...
MaybeLegacyEncodedResult try_open_maybe_legacy_encoded_file(const char *filename, const char *encoding_header_1, const char *encoding_header_2, const char *payload_header_1, const char *payload_header_2)
{
	...
	char id[32];
	id[0] = id[31] = '\0';
	PACKFILE* pf = zalleg_pack_fopen_password(filename, F_READ_PACKED, "");
	if (!pf)
		pf = zalleg_pack_fopen_password(filename, F_READ_PACKED, packfile_password = datapwd);
	if (pf)
	{
	...
	if (strstr(id, payload_header_1) || (payload_header_2 && strstr(id, payload_header_2)))
	{
		...
		{
			result.decoded_pf = zalleg_pack_fopen_password(filename, F_READ_PACKED, packfile_password);
			return result;
		}
		else
		{
			result.decoded_pf = zalleg_pack_fopen_password(filename, F_READ, "");
			return result;
		}
	}
