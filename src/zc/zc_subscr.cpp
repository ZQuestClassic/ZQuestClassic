...
void dosubscr()
{
	...
	for(int32_t y = -distance; y <= 0; y += 3*Hero.subscr_speed)
	{
		...
		if(CURRENT_ACTIVE_SUBSCREEN->script && FFCore.doscript(ScriptType::EngineSubscreen,0))
			ZScriptVersion::RunScript(ScriptType::EngineSubscreen, CURRENT_ACTIVE_SUBSCREEN->script, 0);
		do_dcounters();
		Hero.refill();
		if(CURRENT_ACTIVE_SUBSCREEN->script && FFCore.doscript(ScriptType::EngineSubscreen,0) && FFCore.waitdraw(ScriptType::EngineSubscreen,0))
		{
			ZScriptVersion::RunScript(ScriptType::EngineSubscreen, CURRENT_ACTIVE_SUBSCREEN->script, 0);
...
void put_passive_subscr(BITMAP *dest,int32_t x,int32_t y,bool showtime,int32_t pos2)
{
	...
	// using an extended height viewport). Otherwise the portion of the region behind the passive
	// subscreen would be visible.
	// Previously, custom but blank subscreens could be used to allow scripts to draw to the entire
	// screen (ex: yuurand), but post-z3 such use cases should enable the "no subscreen" screen
	// flag.
