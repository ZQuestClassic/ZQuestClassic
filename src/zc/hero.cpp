...
void HeroClass::check_conveyor()
{
	...
	if((!get_qr(qr_BROKEN_CONVEYOR_WATER) && action==rafting || (ladderx+laddery)) || action==casting||action==sideswimcasting||action==drowning || action==sidedrowning||action==lavadrowning||inlikelike||pull_hero||((z>0||fakez>0) && !(tmpscr->flags2&fAIRCOMBOS)))
	{
		is_conveyor_stunned = 0;
		return;
	}
