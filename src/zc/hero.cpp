...
static bool active_invincible_divineprot(bool for_kb)
{
	...
	if (!for_kb)
		return true;
	auto& itm = get_item_data(div_prot_item);
	if (itm.type != itype_divineprotection)
		return false;
...
bool usingActiveShield(int32_t itmid)
{
	...
	auto const& itm = itemsbuf.get(itmid);
...
int32_t refreshActiveShield()
{
	...
	{
		itemdata const& dat = get_item_data(Bwpn.id);
	...
	{
		itemdata const& dat = get_item_data(Awpn.id);
		if (dat.type == itype_shield && (dat.flags & item_flag9))
		{
			id = Awpn.id;
		}
	...
	{
		itemdata const& dat = get_item_data(Xwpn.id);
		if (dat.type == itype_shield && (dat.flags & item_flag9))
		{
			id = Xwpn.id;
		}
	...
	{
		itemdata const& dat = get_item_data(Ywpn.id);
		if (dat.type == itype_shield && (dat.flags & item_flag9))
		{
			id = Ywpn.id;
		}
	...
	if (id < 0)
	{
		...
		itemdata const& dat = get_item_data(shield_id);
		if (dat.type == itype_shield && (dat.flags & item_flag9))
		{
...
void HeroClass::set_liftflags(int liftid)
{
	if(invalid_item_id(liftid))
		return;
	itemdata const& itm = itemsbuf.get(liftid);
...
int32_t get_grav_boots_id()
{
	...
	if (valid_item_id(grav_id = current_item_id(itype_gravity_boots, true)))
		if (checkmagiccost(grav_id) && checkbunny(grav_id))
			if (!(itemsbuf.get(grav_id).flags & item_flag1) || Hero.sideview_mode()) // sideview-only boots only work in sideview
				return (Hero.last_grav_boots_id = grav_id);
	return -1;
}
zfix HeroClass::get_gravity(bool skip_custom) const
{
	...
	{
		itemdata const& itm = itemsbuf.get(last_rocs_id);
		if (itm.flags & item_flag2)
		{
	...
	if (valid_item_id(grav_id))
		return zslongToFix(itemsbuf.get(grav_id).misc1);
...
zfix HeroClass::get_terminalv(bool skip_custom) const
{
	...
	int32_t grav_id = get_grav_boots_id();
	if (valid_item_id(grav_id))
		return zslongToFix(itemsbuf.get(grav_id).misc2);
...
void HeroClass::set_respawn_point(bool setwarp)
{
	...
	do
	{
		if(!get_qr(qr_OLD_RESPAWN_POINTS))
		{
			...
			if(sideview_mode() && !on_sideview_solid_oldpos(this,true)) break; //in air sideview
...
void HeroClass::force_ice_velocity(optional<zfix> vx, optional<zfix> vy)
{
	...
	if(script_ice_combo && ic && !sliding)
	{
		...
		sliding = 1;
		ice_vx = ice_vy = 0;
		ice_entry_count = ice_entry_mcount = icecmb.c_attributes[9].getTrunc();
	}
	zfix cap = zc_max(0.0001_zf,abs(icecmb.c_attributes[2]));
...
void HeroClass::go_respawn_point()
{
	...
	{
		FFCore.warp_player(wtIWARP, respawn_dmap, respawn_scr-DMaps[respawn_dmap].xoff,
...
void HeroClass::setAction(actiontype new_action) // Used by ZScript
{
	...
	if(valid_item_id(magicitem) && itemsbuf.get(magicitem).type==itype_divineescape)
	{
		// Using Divine Escape
		if(magiccastclk<96)
		{
	...
	case isdiving:
		if(action==swimming && !diveclk)
		{
			...
			set_dive(valid_item_id(flippers_id) ? itemsbuf.get(flippers_id).misc1 : 50);
		}
		return;
...
void HeroClass::draw_under(BITMAP* dest)
{
	...
	if(action==rafting && valid_item_id(c_raft))
	{
		...
		{
			overtile16(dest, raft_item.tile, x - viewport.x, y+playing_field_offset+4 - viewport.y,
	...
	if(ladderx+laddery && valid_item_id(c_ladder))
	{
		...
		{
			overtile16(dest, ladder_item.tile, ladderx - viewport.x, laddery+playing_field_offset - viewport.y,
...
int32_t HeroClass::weaponattackpower(int32_t itid)
{
	...
	if (invalid_item_id(itid))
		return 0;
	auto const& itm = itemsbuf.get(itid);
	...
	if(spins > 0)
	{
		...
		if (valid_item_id(screen))
			power *= itemsbuf.get(screen).power;
	}
...
void HeroClass::positionNet(weapon *w, int32_t itemid)
{
	...
	int32_t clock = (itm.flags&item_flag2 ? (NET_CLK_TOTAL-1)-attackclk : attackclk);
...
void HeroClass::positionSword(weapon *w, int32_t itemid)
{
    ...
    case up:
        ...
        if(game->get_canslash() && w->id==wSword && itm.flags & item_flag4 && charging==0)
        {
...
void HeroClass::draw(BITMAP* dest)
{
	...
	bool invisible = invis_timer || (hero_scr->flags3&fINVISHERO);
	{
		...
		auto const& agony_item = get_item_data(agonyid);
		...
		if(attackclk || (action==attacking||action==sideswimattacking))
		{
			...
			*/
			...
			int32_t itemid = (valid_item_id(directWpn) && itemsbuf.get(directWpn).type == itype) ? directWpn : current_item_id(itype);
			...
			|| (attack==wBugNet && attackclk<NET_CLK_TOTAL))
			{
				if(!invisible)
				{
					...
					{
						if ((dir == right && (itm.flags&item_flag2)) || (dir != right && !(itm.flags&item_flag2)))
						{
			...
			if(attack==wHammer) // To do: possibly abstract this out to a positionHammer routine?
			{
				...
				{
					sfx(((iswaterex_z3(MAPCOMBO(x+wx+8,y+wy), -1, x+wx+8, y+wy, true) || COMBOTYPE(x+wx+8,y+wy)==cSHALLOWWATER) && get_qr(qr_MORESOUNDS)) ? WAV_ZN1SPLASH : itm.usesound,pan(x));
				}
...
void HeroClass::reposition_sword()
{
	weapon *swd = nullptr;
	for (int32_t i = 0; i < Lwpns.Count(); i++)
	{
		...
		{
			int32_t itype = (attack == wFire ? itype_candle : attack == wCByrna ? itype_cbyrna : attack == wWand ? itype_wand : attack == wHammer ? itype_hammer : attack == wBugNet ? itype_bugnet : itype_sword);
			int32_t item_id = (valid_item_id(directWpn) && itemsbuf.get(directWpn).type == itype) ? directWpn : current_item_id(itype);
...
bool HeroClass::checkstab()
{
	...
	if(game->get_canslash() && (attack==wSword || attack==wWand) && itm.flags & item_flag4)
	{
		switch(w->dir)
		{
		case up:
	...
	for(int32_t i=0; i<guys.Count(); i++)
	{
		...
		if((guys.spr(i)->hit(wx,wy,wz,wxsz,wysz,wz>0?16:8) && ((attack!=wWand && attack!=wHammer && attack!=wCByrna) || !(itm.flags & item_flag3)))
				|| ((attack==wWand || attack==wCByrna) && guys.spr(i)->hit(wx,wy-8,z,16,24,z>8) && !(itm.flags & item_flag3))
				|| (attack==wHammer && guys.spr(i)->hit(wx,wy-8,z,16,24,z>0?16:8) && !(itm.flags & item_flag3)))
		{
			...
			{
				if(!(zc_oldrand()%zc_max(itemsbuf.get(whimsyid).misc1,1)))
			...
			{
				auto const& atkring = itemsbuf.get(atkringid);
				dmg *= atkring.misc2; //Multiplier
				dmg += atkring.misc1; //Additive
			}
			...
			{ 
				...
				if (w->parentitem > -1)
					e->hitby[HIT_BY_LWEAPON_PARENT_FAMILY] = get_item_data(w->parentitem).type; 
				else
					e->hitby[HIT_BY_LWEAPON_PARENT_FAMILY] = -1;
			...
			{
				sfx(itemsbuf.get(whimsyid).usesound);
			}
	...
	if(attack == wBugNet
		|| invalid_item_id(parentitem) ? !get_qr(qr_NOITEMMELEE) : !(itemsbuf.get(parentitem).flags & item_flag7))
	{
		...
		for(int32_t j=0; j<items.Count(); j++)
		{
			...
			bool dofairy = (attack==wBugNet && itemsbuf.get(ptr->id).type == itype_fairy)
				&& (valid_item_id(bugnetid) && !(itemsbuf.get(bugnetid).flags & item_flag1));
			
			if((itemsbuf.get(ptr->id).type == itype_bottlefill || dofairy) && !game->hasBottle(0))
			...
			if((ptr->pickup & ipCANGRAB) || (ptr->pickup & ipTIMER) || dofairy)
			{
				if(((ptr->pickup & ipCANGRAB) || ptr->clk2 >= game->get_item_spawn_flicker() || dofairy) && !ptr->fallclk && !ptr->drownclk)
				{
							...
							|| (attack==wHammer && ptr->hit(x,y-8-fakez,z,wxsz,wysz,1)))
					{
						...
						if(dofairy)
						{
							game->fillBottle(itemsbuf.get(ptr->id).misc4);
						}
						else
						{
...
void HeroClass::check_slash_block_layer(int32_t bx, int32_t by, int32_t layer)
{
    ...
    if((get_bit(screengrid_layer[layer-1], i) != 0) || (!isCuttableType(type)))
		return;
	if (!get_qr(qr_OLD_BRIDGE_COMBO_COVER) && has_bridge_above(fx, fy, layer))
		return;

    int32_t sworditem = (valid_item_id(directWpn) && itemsbuf.get(directWpn).type == itype_sword) ? itemsbuf.get(directWpn).level : current_item(itype_sword);
	...
	else if(isCuttableItemType(type))
	{
		...
		//select_dropitem( (combobuf[MAPCOMBO(bx,by)-1].usrflags&cflag2) ? combobuf[MAPCOMBO(bx,by)-1].c_attributes[1].getTrunc() : 12, bx, by);
	...
	if(get_qr(qr_MORESOUNDS))
	{
		if (!isBushType(type) && !isFlowersType(type) && !isGrassType(type))
		{
			...
			{
				sfx(combobuf[cid].c_attributes[10].getTrunc(),pan(bx));
			}
		}
		else
		{
...
void HeroClass::check_slash_block(int32_t bx, int32_t by)
{
	...
	if(get_bit(screengrid, i) != 0)
	{
		...
		ignorescreen = true;
	else if(!isCuttableType(type) &&
			(flag<mfSWORD || flag>mfXSWORD) &&  flag!=mfSTRIKE && (flag2<mfSWORD || flag2>mfXSWORD) && flag2!=mfSTRIKE)
	{
		ignorescreen = true;
	}
	else if (!get_qr(qr_OLD_BRIDGE_COMBO_COVER) && has_bridge_above(fx, fy, 0))
		ignorescreen = true;
	...
	if(!ignorescreen)
	{
		...
		if(get_qr(qr_MORESOUNDS))
		{
			if (!isBushType(type) && !isFlowersType(type) && !isGrassType(type))
			{
				...
				{
					sfx(cmb.c_attributes[10].getTrunc(),pan(bx));
				}
			}
			else
			{
	...
	if(!ignoreffc)
	{
		...
		if(isCuttableItemType(type2))
		{
			...
			{
				auto id = cmb_ff.c_attributes[9].getTrunc();
			...
			if(valid_item_id(it) && itemsbuf.get(it).type != itype_misc) // Don't drop non-gameplay items
			{
		...
		if(get_qr(qr_MORESOUNDS))
		{
			if (!isBushType(type2) && !isFlowersType(type2) && !isGrassType(type2))
			{
				...
				{
					sfx(cmb_ff.c_attributes[10].getTrunc(),pan(bx));
				}
			}
			else
			{
...
void HeroClass::check_slash_block_layer2(int32_t bx, int32_t by, weapon *w, int32_t layer)
{
	...
	if((flag==mfARMOS_ITEM||flag2==mfARMOS_ITEM) && (!getmapflag(s, (cur_screen < 128 && get_qr(qr_ITEMPICKUPSETSBELOW)) ? mITEM : mSPECIALITEM) || (s->flags9&fBELOWRETURN)))
		...
		int16_t decotype = (combobuf[cid].usrflags & cflag1) ? ((combobuf[cid].usrflags & cflag10) ? (combobuf[cid].c_attributes[8].getTrunc()) : (-1)) : (0);
		...
		switch(decotype)
		{
			...
			case -1:
				decorations.add(new comboSprite((zfix)fx, (zfix)fy, dCOMBOSPRITE, 0, combobuf[cid].c_attributes[8].getTrunc()));
				break;
...
void HeroClass::check_slash_block2(int32_t bx, int32_t by, weapon *w)
{
	...
	if (!ignorescreen && !get_qr(qr_OLD_BRIDGE_COMBO_COVER) && has_bridge_above(fx, fy, 0))
		ignorescreen = true;
...
void HeroClass::check_slash_block(weapon *w)
{
	...
	{
		usewpn = itemsbuf.get(par_item).weap_data.imitate_weapon;
	}
    ...
    int32_t sworditem = (valid_item_id(par_item)) ? itemsbuf.get(par_item).level : current_item(itype_sword); //Get the level of the item, else the highest sword level in inventory.
    ...
    if(!ignoreffc)
    {
        ...
        if(isCuttableItemType(type2))
        {
			...
			{
				auto id = combobuf[MAPCOMBO(bx,by)-1].c_attributes[9].getTrunc();
...
void HeroClass::check_pound_block_layer(int bx, int by, int lyr, weapon* w)
{
	...
	if (!get_qr(qr_OLD_BRIDGE_COMBO_COVER) && has_bridge_above(fx, fy, lyr))
		return;
...
int32_t HeroClass::defend(weapon *w)
{
	...
	switch(def)
	{
		...
		case edBREAKSHIELD: //destroy the player's present shield
		{
			...
			if(get_item_data(itemid).flags & item_edible)
...
bool HeroClass::check_ewpn_collide(weapon* w)
{
	...
	if(invalid_item_id(itemid) || !(checkbunny(itemid) && checkmagiccost(itemid)))
		return true;
	itemdata const& shield = itemsbuf.get(itemid);
...
bool HeroClass::try_lwpn_hit(weapon* w)
{
			...
			&& parent.type != itype_divinefire)
		{
			...
			if(!divineprot)
			{
				...
				if (!get_qr(qr_BROKENHITBY) && w)
				{
					...
					if (w->parentitem > -1)
						sethitHeroUID(HIT_BY_LWEAPON_PARENT_ID, w->parentitem);
					else
						sethitHeroUID(HIT_BY_LWEAPON_PARENT_ID, -1);
					sethitHeroUID(HIT_BY_LWEAPON_PARENT_FAMILY, get_item_data(w->parentitem).type);
				}
			}
	...
	if((parent.flags & item_flag6))
	{
		if(w->id==wBrang || (w->id==wHookshot&&!pull_hero))
		{
			...
			{
				auto const& itm2 = itemsbuf.get(itemid2);
				for(int32_t j=0; j<Ewpns.Count(); j++)
				{
					...
					if(w->hit(t->x+7,t->y+7-t->fakez,t->z,2,2,1))
					{
						...
						if(sh_check(itm2.misc3, itm2.misc4, t->id, reflect, ((weapon*)t)->level&1, false))
						{
							w->dead=1;
							weapon *ew = ((weapon*)t);
							int32_t oldid = ew->id;
	...
	if(hclk==0 && w->id==wWind && w->hit(x+7,y+7-fakez,z,2,2,1) && !fairyclk)
	{
		...
		if (valid_item_id(whistle) && itemsbuf.get(whistle).type==itype_whistle)
			whistleitem = whistle;
...
void HeroClass::checkhit()
{
	...
	if(DivineProtectionShieldClk>0)
	{
		...
		if (valid_item_id(div_prot_item))
		{
			...
			else if(get_qr(qr_MORESOUNDS) && !(DivineProtectionShieldClk&0xF00))
			{
				stop_sfx(itm.usesound);
				cont_sfx(itm.usesound+1);
			}
		}
	}
	...
	if(hclk>0 || inlikelike == 1 || is_autowalking() || action==inwind || action==drowning || action==lavadrowning
	...
	for(int32_t i=0; i<Lwpns.Count(); i++)
		if (try_lwpn_hit((weapon*)Lwpns.spr(i)))
			return;
	...
	if(hit2!=-1)
	{
		...
		if (!divineprot)
		{
			...
			if (lwpnspr)
			{
				...
				if (lwpnspr->parentitem > -1)
					sethitHeroUID(HIT_BY_LWEAPON_PARENT_ID, lwpnspr->parentitem);
				else
					sethitHeroUID(HIT_BY_LWEAPON_PARENT_ID, -1);
				sethitHeroUID(HIT_BY_LWEAPON_PARENT_FAMILY, get_item_data(lwpnspr->parentitem).type);
			}
		}
...
bool HeroClass::checkdamagecombos(int32_t dx1, int32_t dx2, int32_t dy1, int32_t dy2, int32_t layer, bool solid, bool do_health_check) //layer = -1, solid = false, do_health_check = true
{
	...
	
	{
		...
		if ( !(cmb.only_gentrig) && combo_class_buf[cmb.type].modify_hp_amount)
		{
			if(cmb.usrflags&cflag1) 
				hp_mod[0] = -cmb.c_attributes[0].getTrunc();
			else 
	...
	}
	{
		...
		{
			if(cmb.usrflags&cflag1) 
				hp_mod[1] = -cmb.c_attributes[0].getTrunc();
			else 
	...
	}
	{
		...
		{
			if(cmb.usrflags&cflag1) 
				hp_mod[2] = -cmb.c_attributes[0].getTrunc();
			else 
	...
	}
	{
		...
		{
			if(cmb.usrflags&cflag1) 
				hp_mod[3] = -cmb.c_attributes[0].getTrunc();
			else 
	...
	if (has_bridge_above(dx1, dy1, 0)) {hp_mod[0] = 0; hasKB &= ~(1<<0);}
	if (has_bridge_above(dx1, dy2, 0)) {hp_mod[1] = 0; hasKB &= ~(1<<1);}
	if (has_bridge_above(dx2, dy1, 0)) {hp_mod[2] = 0; hasKB &= ~(1<<2);}
	if (has_bridge_above(dx2, dy2, 0)) {hp_mod[3] = 0; hasKB &= ~(1<<3);}
	...
	auto const& boots = get_item_data(itemid);
	
	bool bootsnosolid = boots.flags & item_flag1;
	bool ignoreBoots = boots.flags & item_flag3;
...
int32_t HeroClass::hithero(int32_t hit2, int32_t force_hdir)
{
			...
			((isSideViewHero() && (y+16)-(enemyptr->y)<=14) && falling_oldy<y)))
	{
		...
		auto const& stomp = itemsbuf.get(stompid);
		hit_enemy(hit2,wStomp,stomp.power*game->get_hero_dmgmult(),x,y,0,stompid);
		
		if(stomp.flags & item_flag1)
		{
			fall = -(stomp.misc1);
		}
		
		if(stomp.flags & item_downgrade)
	...
	switch(enemyptr->type)
	{
		...
		case eeWALK:
		{
			...
			auto const& itm = get_item_data(itemid);
			int32_t sworddivisor = ((itm.misc1 & 1) ? itm.power : 1);
			int32_t itemdivisor = ((itm.misc1 & 2) ? itm.power : 1);
			int32_t shielddivisor = ((itm.misc1 & 4) ? itm.power : 1);
			switch(dm7)
			{
				case e7tTEMPJINX:
					if(dm8&e8tSWORD)
				...
				case e7tPERMJINX:
					if (dm8&e8tSWORD)
						if(sworddivisor)
							swordclk = (itm.flags & item_flag1) ? int32_t(150/sworddivisor) : -1;
						
					if (dm8&e8tITEM)
						if(itemdivisor)
							itemclk = (itm.flags & item_flag1) ? int32_t(150/itemdivisor) : -1;
					
					if (dm8&e8tSHIELD)
						if(shielddivisor)
							shieldjinxclk = (itm.flags & item_flag1) ? int32_t(150/shielddivisor) : -1;
...
static void do_refill_waitframe()
{
	...
	if(get_qr(qr_PASSIVE_SUBSCRIPT_RUNS_WHEN_GAME_IS_FROZEN))
	{
		...
		if (FFCore.waitdraw(ScriptType::ScriptedPassiveSubscreen) && DMaps[cur_dmap].passive_sub_scrconfig.script != 0 && FFCore.doscript(ScriptType::ScriptedPassiveSubscreen))
		{
			ZScriptVersion::RunScript(ScriptType::ScriptedPassiveSubscreen, DMaps[cur_dmap].passive_sub_scrconfig.script, cur_dmap);
...
static size_t find_bottle_for_slot(size_t slot, bool unowned=false)
{
	...
	for(int q = 0; q < itemsbuf.capacity(); ++q)
	{
		...
		if(itm.type == itype_bottle && itm.misc1 == slot)
		{
			if(game->get_item(q))
				return q;
			if(unowned)
...
heroanimate_skip_liftwpn:;
		...
		if (get_qr(qr_GRASS_SENSITIVE))
		{
			...
			if(g1 && g2 && g3 && g4)
			{
				...
				if(grasscid)
				{
					...
					{
						decorations.add(new dTallGrass(x, y, dTALLGRASS, 0, cmb.c_attributes[14].getTrunc()));
					}
					int32_t thesfx = cmb.c_attributes[11].getTrunc();
					if (action==walking)
	...
	if(z==0 && fakez==0) //Shallow Water / Custom Walk SFX
	{
		...
		{
			if (action != swimming && action != isdiving && action != drowning && action!=lavadrowning && action!=sidedrowning && action!=rafting && action != falling && !in_wind && !IsSideSwim() && !(ladderx+laddery) && !pull_hero && !walk_through_walls)
			{
					...
					if(waterid)
					{
						...
						if (action==walking)
						{
							...
							if (watercmb.type != cSHALLOWWATER || !get_qr(qr_OLD_SHALLOW_SFX))
								idx = 13;
							sfx_no_repeat(watercmb.c_attributes[idx].getTrunc(),pan((int32_t)x));
						}
					}
				}
			}
	...
	if(can_use_item(itype_hoverboots,i_hoverboots))
	{
		...
		if(!(get_item_data(hoverid).flags & item_flag1))
		{
	...
	{
		itemdata const& last_rocs = itemsbuf.get(current_rocs_jump_id);
		if (fall < 0)
		{
	...
	else if (fall > termv || fakefall > termv)
	{
		if (valid_item_id(last_grav_boots_id) ||
			(valid_item_id(last_rocs_id) && (itemsbuf.get(last_rocs_id).flags & item_flag5)))
		{
			if (fall > termv)
				fall = termv;
			if (fakefall > termv)
						...
						if(switchhookclk==switchhookmaxtime/2) //Perform swaps
						{
							...
							else
							{
										...
										for(int q = max_layer; q > -1; --q)
										{
											...
											if(cmb.type == cSWITCHHOOK) //custom flags and such
											{
												if(cmb.usrflags&cflag3) //Breaks on swap
												{
													...
													auto brspr = cmb.c_attributes[9].getTrunc();
													breakable* br = new breakable(x, y, 0_zf,
														cmb, target_rpos_handle.cset(), it, thedropset, cmb.c_attributes[10].getTrunc(),
														brspr ? -1 : 0, brspr, switchhookclk);
											...
											else if(isCuttableType(cmb.type)) //Break and drop effects
											{
												...
												if(get_qr(qr_MORESOUNDS)) //SFX
												{
													if (cmb.usrflags&cflag3)
													{
														breaksfx = cmb.c_attributes[10].getTrunc();
													}
												...
												breakable* br = new breakable(x, y, 0_zf,
													...
													decotype, cmb.c_attributes[8].getTrunc(), switchhookclk);
												br->switch_hooked = true;
												decorations.add(br);
												hooked_layerbits &= ~(0x101<<q); //this swap completed entirely
												hooked_undercombos[q] = -1;
								...
								else if(switching_object) //Switching an object
								{
									...
									{
										auto const& itm = itemsbuf.get(it->id);
			...
			for(size_t slot = 0; slot < 256; ++slot)
			{
				if(size_t bind = game->get_bottle_slot(slot))
				{
					...
					if(word max = std::max(toFill[0], std::max(toFill[1], toFill[2])))
					{
						...
						if (valid_item_id(itemid))
							sfx(itemsbuf.get(itemid).usesound, pan(x));
...
//Helper function
static void deselectbombsWPN(word& wpos, ButtonItemData& BTNwpn, int32_t& directItemBTN,
...
void HeroClass::deselectbombs(int32_t super)
{
	...
	auto type = (super ? itype_sbomb : itype_bomb);
	for (auto itm : btns)
...
static bool drown_check(int cid)
{
	...
	if (invalid_item_id(flippers_id))
		return true;
	itemdata const& itm = itemsbuf.get(flippers_id);
	newcombo const& cmb = combobuf[cid];
	return (itm.level < cmb.c_attributes[8] || ((cmb.usrflags & cflag1) && !(itm.flags & item_flag3)));
}
...
void HeroClass::doMirror(int32_t mirrorid)
{
	...
	itemdata const& mirror = itemsbuf.get(mirrorid);
...
void HeroClass::drop_liftwpn()
{
	...
	itemdata const& glove = get_item_data(liftid);
...
void HeroClass::do_liftglove(int32_t liftid, bool passive)
{
	...
	itemdata const& glove = itemsbuf.get(liftid);
		...
		for(auto q = max_layer; q > -1; --q)
		{
			...
			if(isSwitchHookable(cmb))
			{
				if(cmb.type == cSWITCHHOOK)
				{
					...
					if(zc_max(1,get_item_data((w && w->parentitem>-1) ? w->parentitem : current_item_id(itype_switchhook)).level) < cmb.c_attributes[8])
	...
	switch(itm.type)
	{
		...
		case itype_bomb: case itype_sbomb:
		{
			...
			if(itm.flags & item_flag4)
			{
				...
				if (valid_item_id(liftid) && (itm.weap_data.lift_level <= itemsbuf.get(liftid).level))
				{
		...
		case itype_wand:
		{
			...
			{
				auto const& book = itemsbuf.get(bookid);
				if (book.flags & item_flag4)
					sfx(book.misc2,pan(wx));
				else 
...
bool HeroClass::can_be_used(int itmid)
{
	...
	if (on_cooldown(itmid))
		return false;
	if (IsSideSwim() && (itemsbuf.get(itmid).flags & item_sideswim_disabled))
		return false;
	return true;
}
...
bool HeroClass::doattack()
{
	...
	int32_t bugnetid = (valid_item_id(directWpn) && itemsbuf.get(directWpn).type == itype_bugnet) ? directWpn : current_item_id(itype_bugnet);
	...
	int32_t candleid = (valid_item_id(directWpn) && itemsbuf.get(directWpn).type == itype_candle) ? directWpn : current_item_id(itype_candle);
	int32_t byrnaid = (valid_item_id(directWpn) && itemsbuf.get(directWpn).type == itype_cbyrna) ? directWpn : current_item_id(itype_cbyrna);
	...
	if((attack==wCatching && attackclk>4)||(attack!=wWand && attack!=wSword && attack!=wHammer
											&& (attack!=wFire || (valid_item_id(candleid) && !(itemsbuf.get(candleid).wpn_sprites[0])))
											&& (attack!=wCByrna || (valid_item_id(byrnaid) && !(itemsbuf.get(byrnaid).wpn_sprites[0])))
	...
	if (valid_item_id(itemid))
	{
		auto const& charge_ring = itemsbuf.get(itemid);
		if (charge_ring.misc1 > 0)
			normalcharge = charge_ring.misc1;
		if (charge_ring.misc2 > normalcharge)
			magiccharge = charge_ring.misc2;
	}
	...
	if(doCharge)
	{
		...
		if(charging==normalcharge)
		{
			...
			{
				auto const& scroll = itemsbuf.get(scrollid);
				if(!(scroll.flags&item_flag1))
					paymagiccost(scrollid);
				sfx(scroll.usesound2,pan(x));
			}
		}
		else if(charging==magiccharge)
		{
			...
			{
				auto const& scroll = itemsbuf.get(scroll2id);
				if(!(scroll.flags&item_flag1))
	...
	{
		if(!(itemsbuf.get(byrnaid).wpn_sprites[0]))
		{
			attack = wNone;
		...
		if(charging>=normalcharge && (attack!=wSword || attackclk>=SWORDCHARGEFRAME) && !tapping)
		{
			...
			else if(attack==wHammer && sideviewhammerpound())
			{
				...
				if(paid)
				{
					...
					int hmrid = (valid_item_id(directWpn) && itemsbuf.get(directWpn).type == itype_hammer) ? directWpn : current_item_id(itype_hammer);
					int hmrlvl = invalid_item_id(hmrid) ? 1 : itemsbuf.get(hmrid).level;
	...
	if(attackclk==1 && attack==wFire && valid_item_id(candleid) && !(itemsbuf.get(candleid).wpn_sprites[0]))
	{
	...
	if(attackclk==13 || (attackclk==7 && spins>1 && attack != wHammer && crossid >=0 && checkbunny(crossid) && checkmagiccost(crossid)))
	{
		...
		int32_t wpnid = (valid_item_id(directWpn) && itemsbuf.get(directWpn).type == itype_sword) ? directWpn : current_item_id(itype_sword);
		...
		if (valid_item_id(perilid) && valid_item_id(wpnid)
			&& game->get_life() <= itemsbuf.get(perilid).misc1 * game->get_hp_per_heart()
			&& checkbunny(perilid) && checkmagiccost(perilid)
			// Must actually be able to shoot sword beams
			&& ((itm.flags & item_flag1) || itm.misc1 <= game->get_maxlife()/game->get_hp_per_heart()))
			perilbeam = true;
		...
		if(attack==wFire && valid_item_id(candleid) && itemsbuf.get(candleid).wpn_sprites[0]) // Flame if the Candle stab animation WAS used.
			startwpn(attackid);
			
		if(attack==wCByrna && valid_item_id(byrnaid) && itemsbuf.get(byrnaid).wpn_sprites[0]) // Beam if the Byrna stab animation WAS used.
			startwpn(attackid);
	}
...
bool HeroClass::can_attack()
{
    ...
    if(action==hopping || action==swimming || action==freeze || action==sideswimfreeze
		...
		|| ((action==attacking||action==sideswimattacking)
			&& ((attack!=wSword && attack!=wWand) || !(get_item_data(currentSwordOrWand).flags & item_flag5))
			&& charging!=0))
    {
        return false;
    }
...
{
	bool enabled = valid_item_id(l_id) && (itemsbuf.get(l_id).flags & item_flag6);
...
void do_lens()
{
	...
	if (valid_item_id(itemid))
	{
		...
		if(isWpnPressed(itype_lens) && checkitem_jinx(itemid) && !lensclk && checkbunny(itemid) && checkmagiccost(itemid))
		{
			...
			if (itm.magiccosttimer[0])
				lensclk = itm.magiccosttimer[0];
			else lensclk = 12;
		}
		else
		{
...
void HeroClass::tick_diving()
{
	if (diveclk && isSideViewHero() && get_qr(qr_SIDESWIM))
		set_dive(0);
	int32_t flippers_id = current_item_id(itype_flippers);
	itemdata const& itm = get_item_data(flippers_id);
	...
	if (diveclk > 0)
	{
		...
		if (cool_down)
			set_dive(valid_item_id(flippers_id) ? -itemsbuf.get(flippers_id).misc2 : -30); // start cooldown
	}
	else if (diveclk < 0)
	{
		set_dive(diveclk + 1);
...
void HeroClass::do_rafting()
{
	...
	int32_t raft_step = (invalid_item_id(raft_item) ? 1 : itemsbuf.get(raft_item).misc1);
...
bool HeroClass::pitslide() //Runs pitslide movement; returns true if pit is irresistable
{
	...
	int32_t sensitivity = combobuf[cmbid].c_attributes[10].getTrunc();
...
bool HeroClass::pitfall()
{
	...
	if(fallclk)
	{
		...
		{
			optional<combined_handle_t> comb_handle;
			if((comb_handle = get_pitfall_handle(cx, cy)) && comb_handle->data() == fallCombo) /*nil*/;
			else if((comb_handle = get_pitfall_handle(x1, y1)) && comb_handle->data() == fallCombo) /*nil*/;
			else if((comb_handle = get_pitfall_handle(x2, y1)) && comb_handle->data() == fallCombo) /*nil*/;
			else if((comb_handle = get_pitfall_handle(x1, y2)) && comb_handle->data() == fallCombo) /*nil*/;
			else if((comb_handle = get_pitfall_handle(x2, y2)) && comb_handle->data() == fallCombo) /*nil*/;
...
bool HeroClass::try_drown()
{
	...
	sfx(combobuf[water].c_attributes[12].getTrunc(), pan(x));
	return true;
}
...
void HeroClass::handle_slide(newcombo const& icecmb, zfix& dx, zfix& dy)
{
	...
	if(!sliding) //just hit the ice
	{
		if(is_inair)
			return;
		sliding = 1;
		zfix start_perc = icecmb.c_attributes[8] / 100_zf;
	...
	else //not the first frame sliding
	{
		if(is_inair)
			sliding = 2;
		zfix accel = zc_max(0.0001_zf, icecmb.c_attributes[0]);
		zfix decel = zc_max(0.0001_zf, icecmb.c_attributes[1]);
...
void HeroClass::mod_steps(std::vector<zfix*>& v)
{
	...
	if(slowcombo && has_bridge_above(x+7, y+8, 0))
		slowcombo = false;
	...
	if (can_combo)
	{
		...
		for (int q = 6; q >= 0; --q)
		{
			...
			if (q > 0 && has_bridge_at(x+7, y+8, q))
...
void HeroClass::moveheroOld()
{
	...
	if(diagonalMovement)
	{
		...
		if(can_use_item(itype_flippers,i_flippers)&&current_item(itype_flippers) >= combobuf[wtrc].c_attributes[8]&&(!(combobuf[wtrc].usrflags&cflag1) || (get_item_data(current_item_id(itype_flippers)).flags & item_flag3))&&!(ladderx+laddery)&&z==0&&fakez==0)
		{
...
bool HeroClass::scr_walkflag(zfix_round zdx,zfix_round zdy,int d2, [[maybe_unused]] bool kb, int* canladder)
{
	if(walk_through_walls) return false;
	...
	else if(solid || isSideViewHero() || get_qr(qr_DROWN))
	{
		...
		{
			int32_t wtrx  = iswaterex_z3(MAPCOMBO(dx,dy), -1, dx,dy);
			if (current_item(itype_flippers) >= combobuf[wtrx].c_attributes[8] && (!(combobuf[wtrx].usrflags&cflag1) || (get_item_data(current_item_id(itype_flippers)).flags & item_flag3))) //Don't swim if the water's required level is too high! -Dimi
			{
				...
				if(d2>=left)
				{
					if (wtrx)
					{
						...
						if((replay_version_check(26)) || (abs((dy)-(int32_t(y+c)))<=(b)))
						{
							...
							int32_t lx=dx&0xF0;
							if(current_item(itype_flippers) && current_item(itype_flippers) >= combobuf[iswaterex_z3(MAPCOMBO(lx+8, y+8), -1, lx+8, y+8)].c_attributes[8] && z==0 && fakez==0)
							{
...
void HeroClass::start_auto_walk(const combined_handle_t& target)
{
	...
	const ComboView_CutsceneEffect_PlayerWalk cv{cmb};
	zfix dx = cv.dest_x();
	zfix dy = cv.dest_y();
	if(cmb.usrflags & cflag1)
	{
...
HeroClass::WalkflagInfo HeroClass::walkflag(int32_t wx,int32_t wy,int32_t cnt,byte d2)
{
    ...
    else if(wf || isSideViewHero() || get_qr(qr_DROWN))
    {
		...
		if (current_item(itype_flippers) >= combobuf[wtrx8].c_attributes[8] && (!(combobuf[wtrx8].usrflags&cflag1) || (get_item_data(current_item_id(itype_flippers)).flags & item_flag3))) //Don't swim if the water's required level is too high! -Dimi
		{
		//ladder ignores water combos that are now walkable thanks to flippers -DD
		    unwalkablex = unwalkablex && (!wtrx);
...
void HeroClass::checkpushblock()
{
	...
	for(int lyr = 2; lyr >= 0; --lyr) //Top-down, in case of stacked push blocks
	{
		...
		if(t==cPUSH_HW || t==cPUSH_HEAVY)
			...
			heavy = cmb.c_attributes[8].getTrunc();
		
		if(waitblock && (pushing<16 || hasMainGuy(m->screen))) continue;
		
		if(heavy && (!has_bracelet
			|| bracelet.power < heavy
			|| (limitedpush && usecounts[itemid] >=
				zc_max(1, bracelet.misc3))))
			continue;
		...
		if(t == cPUSHBLOCK && (get_qr(qr_BROKEN_GENERIC_PUSHBLOCK_LOCKING) || (f!=mfPUSHED)))
		{
			...
			{
				int limit = cmb.c_attributes[12+blockdir].getTrunc();
			...
			{
				if(int limit = cmb.c_attributes[12].getTrunc())
				{
...
bool usekey()
{
	...
	else
	{
		...
		{
			run_first_script_of_type(itype_key);
			game->change_keys(-1);
		}
	}
...
void HeroClass::oldchecklockblock()
{
	...
	if((cmb.type==cLOCKBLOCK && !(cmb.only_gentrig) && _effectflag(bx,by,1, -1)) && !has_bridge_above(bx, by, 0))
	{
	...
	{
		auto v = 1 << cmb.c_attributes[13].getTrunc();
		setxmapflag(rpos_handle.screen, v);
		remove_xstatecombos(create_screen_handles(rpos_handle.base_scr), v, false);
	}
	else
	{
	...
	{
		sfx(cmb3.c_attributes[11].getTrunc());
	}
	else sfx(WAV_DOOR);
}
...
void HeroClass::oldcheckbosslockblock()
{
	...
	if((cmb.type==cBOSSLOCKBLOCK && !(cmb.only_gentrig) && _effectflag(bx,by,1, -1)) && !has_bridge_above(bx, by, 0))
	{
		// Context: https://discord.com/channels/876899628556091432/1278165595321405554
		// Layer 0 is overridden by Locked Doors (but only for dungeons) - in that case, checklocked will clear these combos
	...
	else if (cmb2.type==cBOSSLOCKBLOCK && !(cmb2.only_gentrig) && _effectflag(bx2,by,1, -1) && !has_bridge_above(bx2, by, 0))
	{
	...
	{
		auto v = 1 << cmb.c_attributes[13].getTrunc();
		setxmapflag(cmb_screen_index, v);
		remove_xstatecombos(create_screen_handles(scr), v);
	}
	else
	{
	...
	sfx(combobuf[cid].c_attributes[11].getTrunc());
}
...
void HeroClass::oldcheckchest(int32_t type)
{
	...
	if (combobuf[MAPCOMBO(bx,by)].type==type && _effectflag(bx,by,1, -1) && !has_bridge_above(bx, by, 0))
	{
		found=true;
		found_screen_index=get_screen_for_world_xy(bx, by);
	}
	else if (combobuf[MAPCOMBO(bx2,by)].type==type && _effectflag(bx2,by,1, -1) && !has_bridge_above(bx, by, 0))
	{
		found=true;
	...
	if(!found)
	{
		for(int32_t i=0; i<2; i++)
		{
			if (combobuf[MAPCOMBO2(i,bx,by)].type==type && _effectflag(bx,by,1, i) && !has_bridge_above(bx, by, i+1))
			{
				found=true;
				found_screen_index=get_screen_for_world_xy(bx, by);
				break;
			}
			else if (combobuf[MAPCOMBO2(i,bx2,by)].type==type && _effectflag(bx2,by,1, i) && !has_bridge_above(bx2, by, i+1))
			{
				found=true;
				found_screen_index=get_screen_for_world_xy(bx2, by);
				break;
...
void HeroClass::checkchest(int32_t type)
{
	...
	if(found<0)
	{
		...
		for(int32_t i = maxlayer-1; i >= 0; --i)
		{
			if (!done_1)
			{
				...
				if(cmb->type==type && !(cmb->only_gentrig) && _effectflag(bx,by,1, i))
				{
					found = MAPCOMBO2(i,bx,by);
				}
				if (has_bridge_at(bx, by, i+1))
				{
					found = -1;
					done_1 = true;
				}
			...
			if (!done_2)
			{
				...
				if(cmb->type==type && !(cmb->only_gentrig) && _effectflag(bx2,by2,1, i))
				{
					found = MAPCOMBO2(i,bx2,by2);
				}
				if (has_bridge_at(bx2, by2, i+1))
				{
					found = -1;
					done_2 = true;
				}
	...
	int32_t intbtn = cmb->c_attributes[10].getTrunc();
	...
	if(intbtn) //
	{
		if(cmb->usrflags & cflag13) //display prompt
		{
			...
			prompt_y = cmb->c_attributes[17].getTrunc();
		}
...
endsigns:
	...
	{
		prompt_combo = cmb.c_attributes[0].getTrunc();
		prompt_cset = cmb.c_attributes[8].getTrunc();
		prompt_x = cmb.c_attributes[16].getTrunc();
		prompt_y = cmb.c_attributes[17].getTrunc();
	}
...
static int lbeam_flag(uint idx)
{
	if (idx)
		return 1 << (idx - 1);
	return ~0;
...
static void handleBeam(spot_t* grid, size_t age, byte spotdir, rpos_t rpos, byte set, bool block, bool refl)
{
	...
	int32_t trigflag = lbeam_flag(set);
	bool doAge = true;
	spot_t f = 0;
	while (rpos < region_max_rpos)
	{
		...
		else switch (typeMap[(int)rpos])
		{
			...
			case cMIRRORNEW:
			{
				...
				newcombo const& cmb = combobuf[cid];
				byte newdir = cmb.c_attributes[8+spotdir].getTrunc();
				if(newdir > 7) return;
				if(newdir > 3) break;
				spotdir = newdir;
				break;
...
static int get_beamid(newcombo const& cmb)
{
	...
	if(cmb.usrflags&cflag1) //use tile/cset style
		return std::max(0,cmb.c_attributes[0].getTrunc())|(cmb.c_attributes[9].getTrunc()%12)<<24;
	else //use 3-color style
	{
		auto id = -((cmb.c_attributes[11].getTrunc()<<16)|(cmb.c_attributes[10].getTrunc()<<8)|(cmb.c_attributes[9].getTrunc()));
...
static bool launch_lightbeam(const rpos_handle_t& rpos_handle, std::map<int32_t, spot_t*>& maps, bool refl, bool block)
{
	...
	byte spotdir = vbound(cmb.c_attributes[8].getTrunc(), 255, 0);
	auto trigflag_idx = cmb.c_attributes[12].getTrunc();
	if(spotdir > 3)
	{
		grid[(int)rpos_handle.rpos] |= SP_VISITED;
		istrig[(int)rpos_handle.rpos] |= lbeam_flag(trigflag_idx);
	}
	if(refl && rpos_handle.rpos == beam_hero_rpos)
	{
		spotdir = Hero.getDir();
	}
	handleBeam(grid, 0, spotdir, rpos_handle.rpos, trigflag_idx, block, refl);
	return true;
}
	...
	std::map<int32_t, std::map<dword,spot_t>>& ffmaps, bool refl, bool block)
{
	...
	if(spotdir > 3 && sxy.valid())
	{
		...
		istrig[(int)sxy.pos()] |= lbeam_flag(trigflag_idx);
	}
	int beamwid = cmb.c_attributes[13].getTrunc();
	if (beamwid < 1) beamwid = 8;
	...
	handleFFBeam(grid, 0, spotdir, sxy, trigflag_idx, block, refl, beamwid);
	return true;
}
...
void HeroClass::handleSpotlights()
{
	...
	if (cpos_exists_spotlight())
	{
		...
		bool block = !refl && valid && (shield.misc1 & sh_lightbeam);
				...
				for(size_t pos = 0; pos < 176; ++pos)
				{
					...
					switch(cmb.type)
					{
						...
						case cMAGICPRISM: case cMAGICPRISM4:
							if(!cmb.c_attributes[0] || (cmb.c_attributes[0].getZLong() & sh_lightbeam))
							{
	...
	for_every_rpos([&](const rpos_handle_t& rpos_handle) {
		...
		if (mini_cmb.target)
		{
			...
			bool trigged = istrig[(int)rpos_handle.rpos] & lbeam_flag(trigflag_idx);
		...
		{
			trig_each_combo_trigger(rpos_handle, [&](combo_trigger const& trig){
				int32_t trigflag = lbeam_flag(trig.triglbeam);
	...
	for_every_ffc([&](const ffc_handle_t& ffc_handle) {
		...
		if (mini_cmb.target)
		{
			...
			bool trigged = istrig[(int)rpos] & lbeam_flag(trigflag_idx);
			if(cmb.usrflags&cflag2) //Invert
				trigged = !trigged;
			if(cmb.usrflags&cflag1) //Solved Version
			{
...
void HeroClass::checkspecial2(int32_t *ls)
{
	...
	for(int32_t i=bigHitbox?0:8; i<16; i+=bigHitbox?15:7)
	{
		for(int32_t j=0; j<16; j+=15) for(int32_t k=0; k<2; k++)
		{
			...
			else
			{
				...
				scr = rpos_handle.scr;
				if (!get_qr(qr_OLD_BRIDGE_COMBO_COVER) && has_bridge_above(x+j, y+i, 0))
					bridge_covered = true;
			}
	...
	for(int32_t i=0; i<4; i++)
	{
		...
		{
			index=0;
			warpsfx2 = combobuf[cids[i]].c_attributes[8].getTrunc();
		}
		...
		if(types[i]==cSTEP) warpsfx2 = combobuf[cids[i]].c_attributes[8].getTrunc();
		else if(types[i]==cSTEPSAME) { types[i]=cSTEP; warpsfx2 = combobuf[cids[i]].c_attributes[8].getTrunc();}
		else if(types[i]==cSTEPALL) { types[i]=cSTEP; warpsfx2 = combobuf[cids[i]].c_attributes[8].getTrunc(); }
	}
	...
	{ 
		if (COMBOPOS_REGION(tx+8, ty+8)!=stepsecret || get_qr(qr_TRIGGERSREPEAT))
		{
			...
			sfx(combobuf[MAPCOMBO(rpos_handle)].c_attributes[8].getTrunc(),pan((int32_t)x));
	...
	if(type==cSTEP)
	{
		...
		if (next_step != stepnext)
		{
			...
			if (req_item_id && valid_item_id(req_item_id) && !game->get_item(req_item_id))
				...
				; // missing heavy boots
			else if (cmb.type==cSTEP)
			{
				sfx(step_sfx_id,pan((int32_t)x));
				rpos_handle.increment_data();
			}
			else if (cmb.type==cSTEPSAME)
			{
				sfx(step_sfx_id,pan((int32_t)x));
				for_every_rpos([&](const rpos_handle_t& rpos_handle) {
				...
				if (raftflags[m] == mfRAFT || raftflags[m] == mfRAFT_BRANCH)
				{
					if(current_item(itype_raft) && action!=rafting && action!=swimhit && action!=gothit && action!=sideswimhit && z==0 && fakez==0 && (combo_class_buf[COMBOTYPE(tx+8, ty+11)].dock || combo_class_buf[FFCOMBOTYPE(tx+8, ty+11)].dock))
					{
						if((isRaftFlag(nextflag(tx,ty+11,dir,false))||isRaftFlag(nextflag(tx,ty+11,dir,true))))
						{
							...
							raftclk=0;
							if (get_qr(qr_RAFT_SOUND)) sfx(get_item_data(current_item_id(itype_raft)).usesound,pan(x));
...
bool HeroClass::HasHeavyBoots()
{
	...
	{
		auto const& itm = itemsbuf.get(q);
		if ( game->get_item(q) && itm.type == itype_boots && /*iron*/ (itm.flags & item_flag2))
			return true;
	}
	return false;
}
...
bool HeroClass::dowarp(const mapscr* scr, int32_t type, int32_t index, int32_t warpsfx)
{
	...
	switch(type)
	{
		...
		{
			wtype = wtWHISTLE;
			int32_t wind = valid_item_id(whistleitem) ? itemsbuf.get(whistleitem).misc2 : 8;
			int32_t level=0;
	...
	case wtEXIT: // entrance/exit
	{
		...
		warp_update_last_entrance(scr, wrx, wry, wdmap);
	...
	case wtSCROLL:                                          // scrolling warp
	{
		...
		if(!intradmap)
		{
			...
			warp_update_last_entrance(scr, wrx, wry, wdmap, true);
		}
	...
	if(action!=rafting && checkwater && (_walkflag(x,y+(bigHitbox?8:12),0,get_standing_z_state()) || get_qr(qr_DROWN))
			//&& (current_item(itype_flippers) >= combobuf[checkwater].c_attributes[8]) 
		&& (action!=inwind))
	{
		hopclk=0xFF;
		SetSwim();
...
static std::pair<int, int> lookahead_coords([[maybe_unused]] int scrolldir, int x, int y)
{
	x = vbound(x, 0, world_w - 16);
	y = vbound(y, 0, world_h - 16);
	return {x, y};
...
static bool lookaheadraftflag([[maybe_unused]] int scroll_dir, int x, int y)
{
    int cx = x;
    int cy = y + 8;
...
void HeroClass::calc_darkroom_hero(int32_t x1, int32_t y1)
{
	...
	itemdata const& lamp = itemsbuf.get(lampid);
...
static void do_ffc_scroll_layer(BITMAP* dest, int layer, const nearby_scrolling_screens_t& nearby_screens, int xoff, int yoff)
{
	...
	for_every_nearby_screen_during_scroll(nearby_screens, [&](screen_handles_t screen_handles, int, int, int, bool is_new_screen) {
		if (!is_new_screen)
			return;
...
void HeroClass::scrollscr(int32_t scrolldir, int32_t dest_screen, int32_t destdmap)
{
	...
	int ox = 0;
	int oy = 0;
	for_every_nearby_screen_during_scroll(nearby_screens, [&](screen_handles_t, int screen, int offx, int offy, bool is_new_screen) {
...
int32_t HeroClass::ringpower(int32_t dmg, bool noPeril, bool noRing)
{
	...
	if ( get_qr(qr_BROKEN_RING_POWER) )
	{
		...
		if(valid_item_id(itemid) && !noRing)  // current_item_id checks magic cost for rings
		{
			usering = true;
			paymagiccost(itemid);
			if(ring_item.flags & item_flag2)//"Divisor is Percentage Multiplier" flag
			{
				percentage *= ring_item.power/100.0;
			}
			else
			{
				divisor *= ring_item.power;
			}
		}
		...
		if(valid_item_id(itemid) && !noPeril && game->get_life()<=peril_ring_item.misc1*game->get_hp_per_heart() && checkmagiccost(itemid) && checkbunny(itemid))
		{
			usering = true;
			paymagiccost(itemid);
			if(peril_ring_item.flags & item_flag2)//"Divisor is Percentage Multiplier" flag
			{
				percentage *= peril_ring_item.power/100.0;
			}
			else
			{
				divisor *= peril_ring_item.power;
			}
		}
	...
	else
	{
		...
		if(valid_item_id(itemid) && !noRing)  // current_item_id checks magic cost for rings
		{
			...
			if(ring_item.flags & item_flag2)//"Divisor is Percentage Multiplier" flag
			{
				double perc = ring_item.power/100.0;
		...
		if(valid_item_id(itemid) && !noPeril && game->get_life()<=peril_ring_item.misc1*game->get_hp_per_heart() && checkmagiccost(itemid) && checkbunny(itemid))
		{
			...
			if(peril_ring_item.flags & item_flag2)//"Divisor is Percentage Multiplier" flag
			{
				double perc = peril_ring_item.power/100.0;
				if(perc < 0) perc = -perc + 1; //Negative percentage = that percent MORE damage -V
				percentage *= perc;
			}
			else
			{
				if(peril_ring_item.power < 0)
					divisor /= -(peril_ring_item.power);
				else divisor *= peril_ring_item.power;
			}
		}
...
bool checkbunny(int32_t itemid)
{
	...
	if (invalid_item_id(itemid))
		return false;
	return itemsbuf.get(itemid).flags & item_bunny_enabled;
}
...
bool usesSwordJinx(int32_t itemid)
{
	if (invalid_item_id(itemid))
		return false;
	itemdata const& it = itemsbuf.get(itemid);
...
bool checkitem_jinx(int32_t itemid)
{
	if (valid_item_id(itemid))
	{
		...
		if (it.type == itype_shield)
		{
			...
			// Active Shields should also check sword/item jinx flags
			// Passive ones can early-return
			if (!(it.flags & item_flag9))
				return true;
		}
	}
	if (usesSwordJinx(itemid))
		return HeroSwordClk() == 0;
...
{
	if ((itype == Bwpn.get_family()) && getInput(btnB, INPUT_DRUNK | INPUT_HERO_ACTION)) return true;
	if ((itype == Awpn.get_family()) && getInput(btnA, INPUT_DRUNK | INPUT_HERO_ACTION)) return true;
	if ((itype == Xwpn.get_family()) && getInput(btnEx1, INPUT_DRUNK | INPUT_HERO_ACTION)) return true;
	if ((itype == Ywpn.get_family()) && getInput(btnEx2, INPUT_DRUNK | INPUT_HERO_ACTION)) return true;
    return false;
}

int32_t getWpnPressed(int32_t itype)
{
	if ((itype == Bwpn.get_family()) && getInput(btnB, INPUT_DRUNK | INPUT_HERO_ACTION)) return Bwpn.id;
	if ((itype == Awpn.get_family()) && getInput(btnA, INPUT_DRUNK | INPUT_HERO_ACTION)) return Awpn.id;
	if ((itype == Xwpn.get_family()) && getInput(btnEx1, INPUT_DRUNK | INPUT_HERO_ACTION)) return Xwpn.id;
	if ((itype == Ywpn.get_family()) && getInput(btnEx2, INPUT_DRUNK | INPUT_HERO_ACTION)) return Ywpn.id;
...
int32_t getRocsPressed()
{
	...
	if ((itype_rocs == Bwpn.get_family()) && getInput(btnB, INPUT_DRUNK | INPUT_HERO_ACTION))
		return Bwpn.id;
	if ((itype_rocs == Awpn.get_family()) && getInput(btnA, INPUT_DRUNK | INPUT_HERO_ACTION))
		return Awpn.id;
	if ((itype_rocs == Xwpn.get_family()) && getInput(btnEx1, INPUT_DRUNK | INPUT_HERO_ACTION))
		return Xwpn.id;
	if ((itype_rocs == Ywpn.get_family()) && getInput(btnEx2, INPUT_DRUNK | INPUT_HERO_ACTION))
		return Ywpn.id;
...
{
	if (itmid == Bwpn.id && getInput(btnB, INPUT_DRUNK | INPUT_HERO_ACTION)) return true;
	if (itmid == Awpn.id && getInput(btnA, INPUT_DRUNK | INPUT_HERO_ACTION)) return true;
	if (itmid == Xwpn.id && getInput(btnEx1, INPUT_DRUNK | INPUT_HERO_ACTION)) return true;
	if (itmid == Ywpn.id && getInput(btnEx2, INPUT_DRUNK | INPUT_HERO_ACTION)) return true;
    return false;
}

//helper function
static void selectNextBTNWpn(int32_t type, word& wpos, ButtonItemData& BTNwpn,
...
//helper function
static void verifyWpn(word& wpos, ButtonItemData& BTNwpn, int32_t& directItemBTN, word f1 = 255, word f2 = 255, word f3 = 255)
{
	if(!new_subscreen_active)
		return;
	byte pgn = wpos&0xFF, pos = wpos>>8;
...
bool canget(int32_t id)
{
	if (invalid_item_id(id))
		return false;
	return game->get_maxlife() >= (itemsbuf.get(id).pickup_hearts * game->get_hp_per_heart());
}
...
void dospecialmoney(mapscr* scr, int32_t index)
{
    ...
    case rBOMBS:
	{
		...
		{
			auto const& medal = itemsbuf.get(wmedal);
			if(medal.flags & item_flag1)
				price *= (medal.misc1 / 100.0);
			else
				price += medal.misc1;
		}
        ...
        {
			auto const& itm = itemsbuf.get(q);
            if(itm.type == itype_bomb && itm.level == 1)
                getitem(q, true, true);
        }
...
void getitem(int32_t id, bool nosound, bool doRunPassive)
{
	...
	if (replay_is_active())
		replay_step_comment(fmt::format("getitem {}", itm->name));
	
	if(get_qr(qr_SCC_ITEM_COMBINES_ITEMS))
	{
		itemdata const* next_item = nullptr;
		do
		{
			next_item = nullptr;
			if((itm->flags & item_combine) && game->get_item(id))
				// Item upgrade routine.
			{
				size_t next_id = 0;
				for(size_t q = 0; q < itemsbuf.capacity(); ++q)
				{
					auto const& tmp_itm = itemsbuf.get(q);
					// Find the item which is as close to this item's fam_type as possible.
					if (tmp_itm.type == itm->type && tmp_itm.level > itm->level
						&& (!next_item || tmp_itm.level <= next_item->level))
					{
						next_id = q;
						next_item = &tmp_itm;
					}
	...
	if(equipment)
	{
		int cur_itm_id = current_item_id(idat.type);
		auto const& cur_itm = get_item_data(cur_itm_id);
		// Fix sounds.
		switch(idat.type)
		{
			case itype_brang:
...
void takeitem(int32_t id)
{
    ...
    itemdata const& idat = itemsbuf.get(id);
...
{	
	if(index==-1)
	{
		...
		{
			index = find_item_hit(x+itembox_xofs,y-fakez+itembox_yofs,z,itembox_width,itembox_height,1);
		}
	...
	if(itm->type == itype_progressive_itm)
	{
		...
		{
			id2 = newid;
			itm = &get_item_data(id2);
			holdid = newid;
			pstr = itm->pstring;
			pstr_flags = itm->pickup_string_flags;
		}
	}
	...
	{
		if(itm->type == itype_bottlefill && !game->hasBottle(0))
		...
		{
			next_item = nullptr;
			if((itm->flags & item_combine) && game->get_item(id2))
				// Item upgrade routine.
			{
				size_t next_id = 0;
				for(size_t q = 0; q < itemsbuf.capacity(); ++q)
	...
	if(pickup&ipHOLDUP)
	{
		...
		if(itm->type!=itype_triforcepiece || !(itm->flags & item_gamedata))
		{
	...
	if(itm->type == itype_triforcepiece
		&& itm->misc2 <= 0 && linked_parent == eeGANON)
	{
		game->lvlitems[dlevel] |= (1 << li_boss_killed);
	}
...
void HeroClass::StartRefill(int32_t refillWhat)
{
	...
	if(!refilling)
	{
		...
		{
			auto const& itm = itemsbuf.get(refill_why);
			if (itm.type == itype_potion || itm.type == itype_triforcepiece)
			{
				if (itm.flags & item_flag3)
				{
					swordclk = 0;
					verifyAWpn();
				}
	...
	if(!refilling)
	{
		...
		if(valid_item_id(refill_why)) // Item index
		{
			...
			if((itm.type == itype_potion) && (!get_qr(qr_NONBUBBLEMEDICINE)))
			{
				swordclk = 0;
				verifyAWpn();
				if(get_qr(qr_ITEMBUBBLE))
					itemclk = 0;
			}

			if((itm.type == itype_triforcepiece) && (!get_qr(qr_NONBUBBLETRIFORCE)))
			{
				swordclk = 0;
				verifyAWpn();
				if(get_qr(qr_ITEMBUBBLE))
...
bool HeroClass::refill()
{
    ...
    if(valid_item_id(refill_why))
    {
		...
		{
			refill_heart_stop = zc_min(potion_life + (itm.flags & item_flag1 ? int32_t(game->get_maxlife() * (itm.misc1 / 100.0)) : ((itm.misc1 * game->get_hp_per_heart()))), game->get_maxlife());
			refill_magic_stop = zc_min(potion_magic + (itm.flags & item_flag2 ? int32_t(game->get_maxmagic() * (itm.misc2 / 100.0)) : ((itm.misc2 * game->get_mp_per_block()))), game->get_maxmagic());
		}
    }
...
void HeroClass::getTriforce(int32_t id2)
{
	auto const& itm = get_item_data(id2);
	PALETTE flash_pal;
	int32_t refill_frame = ( (itm.misc5 > 0) ? itm.misc5 : 88 );
	...
	sfx(itm.playsound);
	if ( !(itm.flags & item_flag11) ) music_stop();
	
	//If item flag six is enabled, and a sound is set to attributes[2], play that sound.
	if ( (itm.flags & item_flag14) )
	{
		uint8_t playwav = itm.misc3;
		sfx(playwav);
		
	}
		
	//itm.flags & item_flag9 : Don't dismiss Messages
	//itm.flags & item_flag10 : Cutscene interrupts action script..
	//itm.flags & item_flag11 : Don't change music.
	//itm.flags & item_flag12 : Run Collect Script Script On Collection
	//itm.flags & item_flag13 : Run Action Script On Collection
	//itm.flags & item_flag14 : Play second sound (WAV) from Attributes[2] (misc2)
	//itm.flags & item_flag15 : No MIDI
    
	if(!(itm.flags & item_flag15)) //No MIDI flag
	{
		if(itm.misc1)
			jukebox(itm.misc1+MIDIOFFSET_ZSCRIPT);
		else
	...
	do
	{
		...
		//if ( itm.misc2 == 2 ) //No cutscene; what if people used '2' on older quests?
		if ( (itm.flags & item_flag12) ) //No cutscene
		{
			return;
		}
		...
		if(f==40)
		{
			...
			ALLOFF((!(itm.flags & item_flag9)), false);
		...
		if(itm.flags & item_flag1) // Warp out flag
		{
			if(f>=208 && f<288)
			{
				++x2;
	...
	while
	(
		(f < ( (itm.misc4 > 0) ? itm.misc4 : 408)) 
		|| (!(itm.flags & item_flag15) /*&& !(itm.flags & item_flag11)*/ && (midi_pos > 0 && !replay_is_active())) 
		|| (/*!(itm.flags & item_flag15) &&*/ !(itm.flags & item_flag11) && (zcmusic!=NULL) && (zcmusic->position<800 && !replay_is_active())
	...
	if((itm.flags & item_flag1) && ( get_qr(qr_SIDEVIEWTRIFORCECELLAR) ? ( cur_screen < MAPSCRS192b136 ) : (cur_screen < MAPSCRSNORMAL) ) )
	{
		sdir=dir;
	...
	{
		if ( !(itm.flags & item_flag11) ) playLevelMusic();
	}
}
...
void HeroClass::reset_hookshot()
{
	...
	if (valid_item_id(index))
		stop_sfx(itemsbuf.get(index).usesound);
...
void HeroClass::set_dive(int32_t newdive)
{
	if (newdive > 0)
	{
		...
		if (valid_item_id(flippers_id))
			targ_z = -(itemsbuf.get(flippers_id).level);
...
bool HeroClass::on_sideview_solid() const
{
	// This function should not be used for the hero; see 'on_sideview_solid_oldpos' instead
	// Hero has different params and such so this function signature can't handle hero's needs.
	DCHECK(false);
	return false;
}
newcombo const* HeroClass::check_conveyor()
{
	...
	if (cmbid < 0 && !get_qr(qr_BROKEN_SV_SOLID_CONVEYORS) && (isSideViewHero() && on_sideview_solid_oldpos(this)))
	{
		for (int xo = 0; xo < 16 && cmbid < 0; xo += 4)
			cmbid = get_conveyor(x + xo, y + 16, true);
	}
	if(cmbid < 0)
	{
		if (conveyclk <= 0)
	...
	if (!deltax && !deltay && get_qr(qr_BROKEN_SV_SOLID_CONVEYORS)
		&& isSideViewHero() && on_sideview_solid_oldpos(this))
	{
