...
void HeroClass::scrollscr_butgood(int32_t scrolldir, int32_t destscr, int32_t destdmap)
{
	...
	double new_hero_x, new_hero_y;
	{
		...
		// Set the current viewport to be what the final viewport will be... minus some adjustments
		...
		// and only align during the scrolling loop. So translate the "starting" viewport to account
		// for this later alignment.
		if (dx)      viewport.y += secondary_axis_alignment_amount;
		else if (dy) viewport.x += secondary_axis_alignment_amount;
	...
	for(word i = 0; (scroll_counter >= 0 && delay != 0) || align_counter; i++, scroll_counter--) //Go!
	{
		...
		if(!no_move)
		{
			...
			x = vbound(x, new_hero_x - 256, new_hero_x + 256);
			y = vbound(y, new_hero_y - sch, new_hero_y + sch);
		...
		for_every_nearby_screen_during_scroll(old_temporary_screens, [&](mapscr* screens[], int map, int scr, int draw_dx, int draw_dy) {
			...
			if(XOR(base_screen->flags7&fLAYER2BG, DMaps[currdmap].flags&dmfLAYER2BG)) do_layer(scrollbuf, 0, map, scr, 2, base_screen, screens[2], -offx, -offy + playing_field_offset, 2);
			if(XOR(base_screen->flags7&fLAYER3BG, DMaps[currdmap].flags&dmfLAYER3BG)) do_layer(scrollbuf, 0, map, scr, 3, base_screen, screens[3], -offx, -offy + playing_field_offset, 2);
		});
		...
		if(get_bit(quest_rules, qr_NEW_DARKROOM) && get_bit(quest_rules, qr_NEWDARK_L6))
		{
			...
			for_every_nearby_screen_during_scroll(old_temporary_screens, [&](mapscr* screens[], int map, int scr, int draw_dx, int draw_dy) {
				mapscr* base_screen = screens[0];
				if (!(base_screen->flags & fDARK)) return;
