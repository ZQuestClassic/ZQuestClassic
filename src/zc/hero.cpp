...
{
	for (const auto& nearby_screen : nearby_screens)
	{
		fn(nearby_screen.screen_handles, nearby_screen.screen, nearby_screen.dx, nearby_screen.dy, nearby_screen.is_new);
	}
}
...
void HeroClass::check_conveyor()
{
	...
	if(action==casting||action==sideswimcasting||action==drowning || action==sidedrowning||action==lavadrowning||inlikelike||pull_hero||((z>0||fakez>0) && !(hero_scr->flags2&fAIRCOMBOS)))
	{
		is_conveyor_stunned = 0;
		return;
	}
