...
void StunGuy(int32_t j,int32_t stun)
{
    ...
    else if(((enemy*)guys.spr(j))->z==0 && ((enemy*)guys.spr(j))->fakez==0 && ((enemy*)guys.spr(j))->type == eeLEV && get_qr(qr_QUAKE_STUNS_LEEVERS))
    {
        ((enemy*)guys.spr(j))->stunclk=zc_min(360,stun*4);
    }
}
...
void init_game_vars(bool is_cont_game = false)
{
	...
	Hero.init();
	// Not reset by any of the above. A previous in-process game's leftover value is read by
	// walkability checks on the first gameplay frame before it's recomputed, nudging hero
	// movement (and rng-desyncing). Not reset in HeroClass::init() because init() also runs
	// on continue/level-restart, where a reset would desync already-recorded replays.
...
int32_t init_game()
{
	...
	{
		switch (game->get_regionmapping())
		{
			case REGION_MAPPING_FULL:
				for_every_base_screen_in_region([&](mapscr*, unsigned int x, unsigned int y) {
					mark_visited(cur_screen + x + y*16);
				});
				break;
			case REGION_MAPPING_PHYSICAL:
				mark_visited(Hero.current_screen);
				break;
		}
	}
...
static void load_replay_file(ReplayMode mode, std::string replay_file, int frame)
{
	...
	// Older quests don't initialize all sections themselves, so defaults are set for some things by loading the default quest.
...
reload_for_replay_file:
	...
	disable_script_error_logs = used_switch(argc, argv, "-experimental-disable-script-error-logs") > 0;
	FFCore.log_stack_trace_on_trace = is_feature_enabled("-log-stack-trace-on-trace", "ZSCRIPT", "log_stack_trace_on_trace", false);
	hangcount = zc_get_config("ZSCRIPT","ZASM_Hangcount",100000);
