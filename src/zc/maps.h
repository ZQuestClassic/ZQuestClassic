...
struct CameraState
{
	enum class Stage
	{
		...
		Returning,
		Done,
		Parked, // finished with no return; holds position until explicitly cleared
	};
...
enum class CameraEffectSubType
{
	RelativeToCombo,
	RelativeToHero,
	RelativeToViewport,
	...
	First = RelativeToCombo,
	Last = ReturnToHero,
};

enum class CameraEffectCausesMode
{
	OnFinish,
	BeforeReturn,
	BeforeIdle,
...
struct CameraEffect
{
	...
	bool return_to_hero;
	bool freeze_game;
	CameraEffectCausesMode causes_mode;
	CameraState state;
};
...
void draw_screen(bool showhero=true, bool runGeneric = false, bool drawPassiveSubscreenSeparate = false);
...
bool solpush_walkflag(int32_t x, int32_t y, solid_object const* ign = nullptr);
