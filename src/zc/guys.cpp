...
bool enemy::scr_walkflag(int32_t dx,int32_t dy,int32_t special, int32_t dir, int32_t input_x, int32_t input_y, bool kb)
{
	...
	if(!(moveflags & FLAG_IGNORE_SCREENEDGE)
		&& ((input_x<(16-nb)) || (input_y<zc_max(16-yg-nb,0))
			|| (input_x >= (256-16+nb)) || (input_y >= (176-16+nb))))
		return true;
...
bool enemy::animate(int32_t index)
{
	...
	if (can_pitfall() && ((z <= 0 && fakez <= 0 && !isflier(id)) || (isflier(id) && (stunclk))) && !superman)
	{
		...
		if (!(isSideViewGravity() && get_qr(qr_OLD_SPRITE_FALL_DROWN)) && (moveflags & FLAG_CAN_WATERDROWN))
...
{
	if ((moveflags & FLAG_IGNORE_SOLIDITY) && (moveflags & FLAG_USE_NEW_MOVEMENT))
		return false;
	int32_t usewid = (SIZEflags&guyflagOVERRIDE_HIT_WIDTH) ? hit_width : 16;
	int32_t usehei = (SIZEflags&guyflagOVERRIDE_HIT_HEIGHT) ? hit_height : 16;
...
bool enemy::hitshield(int32_t wpnx, int32_t wpny, int32_t xdir)
{
	...
	if(wpnx < x) ret = ret || ((flags&inv_front && xdir==left) || (flags&inv_back && xdir==right) || (flags&inv_left && xdir==down) || (flags&inv_right && xdir==up));
	else if(wpnx > x) ret = ret || ((flags&inv_front && xdir==right) || (flags&inv_back && xdir==left) || (flags&inv_left && xdir==up) || (flags&inv_right && xdir==down));
...
int32_t enemy::takehit(weapon *w, weapon* realweap)
{
	...
	case wBrang:
	{
		...
		if(!(flags & guy_bhit))
		{
			...
			if (get_qr(qr_BROKEN_0_DAMAGE_BRANG_HSHOT))
				power = (enemyHitWeapon>-1 ? itemsbuf[enemyHitWeapon].power : current_item_power(itype_brang)) * game->get_hero_dmgmult();;
	...
	case wHookshot:
	{
		...
		if(swgrab || !(flags & guy_bhit))
		{
			...
			if (get_qr(qr_BROKEN_0_DAMAGE_BRANG_HSHOT))
				power = (enemyHitWeapon>-1 ? itemsbuf[enemyHitWeapon].power : current_item_power(itype_hookshot)) * game->get_hero_dmgmult();;
...
static bool tek_combo_check(zfix tx, zfix ty)
{
	...
	tx = vbound(tx, 0, 256-1);
	ty = vbound(ty, 0, 176-1);
...
void update_slope_combopos(int32_t lyr, int32_t pos)
{
	...
	if(isSlope && !wasSlope)
	{
		slopes.try_emplace(id, &(s->data[pos]), nullptr, COMBOX(pos), COMBOY(pos));
	}
...
static void update_slope_combopos_bordering_screen(int dir, int slope_count, int cid, bool is_slope, int offx, int offy)
{
	...
	if(isSlope && !wasSlope)
	{
		...
		TMP[tmp_index] = cid;
		slopes.try_emplace(id, &TMP[tmp_index], nullptr, offx, offy);
	}
	else if(wasSlope && !isSlope)
	{
		slopes.erase(it);
...
void screen_ffc_modify_postroutine(word index)
{
	...
	if(isSlope && !wasSlope)
	{
		slopes.try_emplace(id, nullptr, &ff);
	}
	else if(wasSlope && !isSlope)
	{
		slopes.erase(it);
		...
		if ( DMaps[currdmap].flags&dmfNEWCELLARENEMIES )
		{
			for(int32_t i=0; i<10; i++)
			{
				if ( tmpscr->enemy[i] )
				{
					...
					addenemy(dngn_enemy_x[i%4],96,tmpscr->enemy[i],-14-i);
					if (guys.Count() > preguycount)
					{
						if (!get_qr(qr_ENEMIES_DONT_SCRIPT_FIRST_FRAME))
						{
