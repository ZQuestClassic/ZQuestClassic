...
void _handle_ffc_layer(BITMAP* bmp, int32_t layer, const screen_handle_t& screen_handle, int32_t x, int32_t y)
{
	if(!show_ffcs) return;
	...
	for(int32_t i = (base_scr->numFFC()-1); i >= 0; --i)
	{
		if (base_scr->ffcs[i].data == 0)
			continue;
		if (layer > -1 && base_scr->ffcs[i].layer != layer)
			continue;
...
void do_ffc_layer(BITMAP* bmp, int32_t layer)
{
	if(!show_ffcs) return;
	for_every_base_screen_in_region([&](mapscr* base_scr, unsigned int region_scr_x, unsigned int region_scr_y) {
		...
		_handle_ffc_layer(bmp, layer, handle, 0, 0);
	});
}
...
static void draw_screen_old(bool showhero, bool runGeneric)
{
	...
	do_ffc_layer(framebuf, 5);
	draw_msgstr(5);
	
	do_ffc_layer(framebuf, -1); // 'overhead' freeform combos
	...
	do_ffc_layer(framebuf, 7);
	draw_msgstr(7);
	
	set_clip_rect(scrollbuf, 0, 0, scrollbuf->w, scrollbuf->h);
	if(runGeneric) FFCore.runGenericPassiveEngine(SCR_TIMING_POST_DRAW);
}
static void draw_sprites_new(BITMAP* dest, set<sprite*, SpriteSorter>& sprite_set, set<sprite*, SpriteSorter>::iterator& it, zfix upto_z)
{
	// Just clips items if in a repeating, smooth maze.
	bool do_clip = maze_state.active && maze_state.loopy;
	...
	while(it != sprite_set.end() && (upto_z < 0 || (*it)->total_z() < upto_z))
	{
		sprite* spr = *it;
...
void draw_screen(bool showhero, bool runGeneric)
{
	...
	if(drawguys)
	{
		...
		bool show_shadow = (get_qr(qr_SHADOWS)&&(!get_qr(qr_SHADOWSFLICKER)||frame&1));
		guys.forEach([&](sprite& spr)
			{
				sorted_sprites.insert(&spr);
				if(show_shadow && spr.total_z() > 0 && ((enemy&)spr).can_drawshadow())
				{
	...
	portals.add_to_set(sorted_sprites);
	if(get_qr(qr_PUSHBLOCK_SPRITE_LAYER))
		sorted_sprites.insert(&mblock2);
	auto sprite_it = sorted_sprites.begin();
	
	draw_sprites_new(framebuf, sorted_sprites, sprite_it, zinit.sprite_z_thresholds[SPRITE_THRESHOLD_GROUND]);
	...
	draw_sprites_new(framebuf, sorted_sprites, sprite_it, zinit.sprite_z_thresholds[SPRITE_THRESHOLD_3]);
	...
	draw_sprites_new(framebuf, sorted_sprites, sprite_it, zinit.sprite_z_thresholds[SPRITE_THRESHOLD_4]);
	...
	draw_sprites_new(framebuf, sorted_sprites, sprite_it, zinit.sprite_z_thresholds[SPRITE_THRESHOLD_OVERHEAD]);
	...
	draw_sprites_new(framebuf, sorted_sprites, sprite_it, zinit.sprite_z_thresholds[SPRITE_THRESHOLD_5]);
	...
	draw_sprites_new(framebuf, sorted_sprites, sprite_it, -1);
