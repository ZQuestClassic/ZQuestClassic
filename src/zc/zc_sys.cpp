		...
		for(int32_t i=0; i<176; i++)
		{
			...
			for(int32_t iter=0; iter<2; ++iter)
			{
						...
						if(!hints)
				{
					...
					case mfDIVE_ITEM:
					{
						int flag = (currscr < 128 && get_qr(qr_ITEMPICKUPSETSBELOW)) ? mITEM : mSPECIALITEM;
						if((!getmapflag(flag) || (tmpscr->flags9&fBELOWRETURN)) && !(itemsbuf[Hero.getLastLensID()].flags & ITEM_FLAG3))
						{
...
void draw_lens_over()
{
	...
	masked_blit(lens_scr, framebuf, w-(HeroX()+8)+viewport.x, h-(HeroY()+8)+viewport.y-playing_field_offset, 0, playing_field_offset, 256, viewport.h);
...
void updatescr(bool allowwavy)
{
	...
	bool nosubscr = GameLoaded && no_subscreen() && !(tmpscr->flags3&fNOSUBSCROFFSET);
...
int32_t onNonGUISnapshot()
{
	...
	if (no_subscreen() && !(tmpscr->flags3&fNOSUBSCROFFSET) && !key[KEY_ALT])
	{
...
static DIALOG quest_dlg[] =
{
	...
	{ jwin_text_proc,		 184,  104,  64,   8,	vc(7),   vc(11),  0,	   0,		 0,		0,	   str_ver, NULL,  NULL },
...
int32_t onQuest()
{
	...
	sprintf(str_ver,"%s",VerStr(QHeader.zelda_version));
