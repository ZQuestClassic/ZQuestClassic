...
bool weapon::no_triggers() const
{
	...
	switch(id)
	{
		case wHammer: //Hammers don't trigger anything while in the air if item_flag1 is set!
		{
			...
			itemdata const& itm = itemsbuf.get(parentitem);
	...
	if ( !checked || (combobuf[cid].usrflags&cflag5) ) 
	{
		...
		if ( (combobuf[cid].usrflags&cflag6) && !getmapflag(scr, mSPECIALITEM) && valid_item_id(scr->catchall))
		{
			items.add(new item(x, y, 0,
				scr->catchall,ipONETIME2|ipBIGRANGE|((itemsbuf.get(scr->catchall).type==itype_triforcepiece ||
	...
	if ( !w->ffcs_checked.contains(ffc) || (combobuf[cid].usrflags&cflag5) ) 
	{
		...
		if ( (combobuf[cid].usrflags&cflag6) && !getmapflag(ffc_handle.scr, mSPECIALITEM) && valid_item_id(ffc_handle.scr->catchall))
		{
			items.add(new item(ffc->x, ffc->y,
				(zfix)0,
				ffc_handle.scr->catchall,ipONETIME2|ipBIGRANGE|((itemsbuf.get(ffc_handle.scr->catchall).type==itype_triforcepiece ||
...
void weapon::cleanup_sfx()
{
    ...
    switch(id)
    {
		...
		case wSSparkle:
		case wFSparkle:
			if(valid_item_id(parentitem) && itemsbuf.get(parentitem).type==itype_cbyrna)
				break;
			return;
    ...
    if(use_sfx)
    {
        for(int32_t i=0; i<Lwpns.Count(); i++)
        {
			...
			{
				auto const& itm = itemsbuf.get(wparent);
				if (itm.type == itype_brang || itm.type == itype_divineprotection
					|| itm.type == itype_hookshot || itm.type == itype_cbyrna)
				{
					if(itm.usesound == use_sfx)
						return;
				}
            }
        }
...
weapon::weapon(zfix X,zfix Y,zfix Z,int32_t Id,int32_t Type,int32_t pow,int32_t Dir, int32_t Parentitem, int32_t prntid, bool isDummy, byte script_gen, byte isLW, byte special, int32_t Linked_Parent, int32_t use_sprite, bool autoRotate) : sprite(), parentid(prntid)
{
	...
	if ( valid_item_id(Parentitem) )
	{
		...
		if ( id != wPhantom /*&& (id != wWind && !specialinfo)*/ && /*id != wFSparkle && id != wSSparkle &&*/ ( id < wEnemyWeapons || ( id >= wScript1 && id <= wScript10) ) )
			level = itemsbuf.get(Parentitem).level; //the weapon level for real lweapons.
		...
		case wSword: // Hero's sword
		{
			...
			else
			{
				...
				if (valid_item_id(itemid))
					ret = itemsbuf.get(itemid).wpn_sprites[0];
				else ret = wSWORD;
			}
			LOADGFX(*ret);
			break;
				...
				switch(parent.type)
				{
					case itype_divinefire: // Divine Fire. This uses magicitem rather than itemid
						if (valid_item_id(magicitem) && !isDummy)
							ret = itemsbuf.get(magicitem).wpn_sprites[4];
						else ret = wFIRE;
						break;
...
{
	if ( get_item_data(parentitem).type == itype_sword && id != wBeam )
		return true;
	return false;
}
...
void weapon::limited_animate()
{
	switch(id)
	{
		...
		case wSBomb:
		{
			...
			{
				sfx(invalid_item_id(parentitem) ? WAV_BOMB : itemsbuf.get(parentitem).usesound,pan(x));
			...
			int32_t boomend = misc+((get_item_data(parentitem).flags & item_flag1) ? 35 : 31);
...
bool weapon::animate([[maybe_unused]] int32_t index)
{
	...
	if(misc_wflags & WFLAG_PICKUP_ITEMS) //Weapon grabs touched items, giving them to the player, similar to engine melee weapons.
	{
		...
		for(int32_t j=0; j<items.Count(); ++j)
		{
			...
			if((itemsbuf.get(ptr->id).type == itype_bottlefill) && !game->hasBottle(0))
	...
	switch(id)
	{
		...
		case wThrown:
			...
			break;
		case wSword:
			if ( get_item_data(parentitem).misc10 == 50 )
			{
				if(runscript_do_earlyret(run_script(MODE_NORMAL))) return false;
			}
		[[fallthrough]];
		...
		case wCByrna:
		{
			...
			int32_t radius = valid_parent ? zc_max(prnt_itm.misc2,8) : 8;
			...
			if(valid_parent && dead != 1) //Perhaps don't play the sound if the weapon is dead?
			{
				cont_sfx(prnt_itm.usesound, pan(x));
			}
			if(runscript_do_earlyret(run_script(MODE_NORMAL))) return false;
		...
		case ewSword:
		{
			...
			if (id != ewSword && valid_item_id(parentitem))
				mirrors = itemsbuf.get(parentitem).flags & item_flag9;
			if(mirrors)
				if(do_mirror())
					break;
		...
		case wWind:
		{
			...
			if (valid_item_id(parentitem))
				mirrors = itemsbuf.get(parentitem).flags & item_flag3;
			if(mirrors)
			{
				do_mirror();
				break;
		...
		case wFire: case wRefFire: case wRefFire2:
		{
			...
			else if(!valid_parent || parent.type != itype_book)
			{
				if(clk==32)
				{
					step=0;
					
					if(!valid_parent || !(parent.flags & item_flag2))
					{
						isLit = true;
						if(!valid_parent ? get_qr(qr_TEMPCANDLELIGHT)
				...
				{
					triggerfire(x,y,this,true,
						true,!valid_parent ? level > 1 : (parent.flags & item_flag9),
			...
			else //book+wand fire
			{
				...
				if(clk==80)
				{
					...
					if((parent.flags & item_flag5) && flame_count()==1)
					{
						isLit=false;
						checkLightSources();
					}
			...
			// Killed by script?
			if(dead==0 && (!valid_parent ? get_qr(qr_TEMPCANDLELIGHT)
				: (parent.flags & item_flag5)) && flame_count()==1)
			{
				isLit=false;
				checkLightSources();
		...
		case wBait:
		{
			...
			if(valid_item_id(parentitem) && clk >= itemsbuf.get(parentitem).misc1)
			{
				dead=1;
			}
			if(runscript_do_earlyret(run_script(MODE_NORMAL))) return false;
		...
		case wBrang:
		{
			...
			{
				stop_sfx(get_item_data(brang_id).usesound);
				if(runscript_do_earlyret(run_script(MODE_NORMAL))) return false;
				break;
			}
			...
			{
				++clk;
				sfx_ex(brangitm.usesound,pan(x),true);
				return false;
			}
			...
			cont_sfx(brangitm.usesound, pan(x));
		...
		case wHookshot:
		{
			...
			if(misc==1)                                           // returning
			{
				if((dir<left && abs(HeroY()-y)<9) || (dir >= left && abs(HeroX()-x)<9))
				{
					...
					if (valid_item_id(parentitem))
						stop_sfx(itemsbuf.get(parentitem).usesound);
		...
		case wMagic:
		{
			...
			{
				itemdata const& book = itemsbuf.get(linkedItem);
...
bool weapon::_prism_dupe(zfix newx, zfix newy, rpos_t cpos, ffc_id_t ffcpos, int tdir)
{
	...
	switch(id)
	{
		...
		case wScript6: case wScript7: case wScript8: case wScript9: case wScript10:
		{
			if (valid_item_id(parentitem) && (itemsbuf.get(parentitem).flags & item_flag1))
			{
				switch(w->dir)
				{
					case up:
...
void weapon::onhit(bool clipped, int32_t special, int32_t linkdir, enemy* e, int32_t ehitType, bool was_pierced)
{
    ...
    case wCByrna:
        // byrna keeps going
        if(!valid_parent || !(prnt_itm.flags & item_flag1))
            dead=0;
	...
	case wBrang:
	{
		...
		if(misc==0)
		{
			int32_t deadval = ((valid_parent ? prnt_itm : get_item_data(current_item_id(itype_brang))).flags & item_flag3) ? -2 : 4;
			clk2=256;
			if(clipped)
				dead=deadval;
			else
			...
			if(family_class==itype_switchhook)
			{
				if(e && !switching_object && (ehitType < 0 || e->switch_hooked))
				{
					...
					if(switching_object)
					{
						...
						misc=2;
						step=0;
						Hero.doSwitchHook(valid_parent ? prnt_itm.misc5 : game->get_switchhookstyle());
						if (valid_parent)
						{
							if (prnt_itm.usesound2)
								sfx(prnt_itm.usesound2, pan(x));
							else if (QMisc.miscsfx[sfxSWITCHED])
								sfx(QMisc.miscsfx[sfxSWITCHED], pan(x));
							stop_sfx(prnt_itm.usesound);
						}
						else if(QMisc.miscsfx[sfxSWITCHED])
							sfx(QMisc.miscsfx[sfxSWITCHED],pan(x));
						break;
	...
	case wMagic:
	{
		...
		auto const& linked_itm = get_item_data(linkedItem);
		if (get_qr(qr_BROKENBOOKCOST))
		{
			int book_id = current_item_id(itype_book);
			auto const& book_itm = get_item_data(book_id);
			if (((id == wMagic && current_item(itype_book) &&
				(book_itm.flags & item_flag1))) && Lwpns.idCount(wFire) < 2)
			{
				Lwpns.add(new weapon(x, y - fakez, z, wFire, 2, zc_max(1, book_itm.misc4) * game->get_hero_dmgmult(), 0, book_id, -1, false, 0, 1));
				if (FFCore.getQuestHeaderInfo(vZelda) < 0x255)
					sfx(WAV_FIRE, pan(x));
				else
					sfx(linked_itm.usesound > 0 ? linked_itm.usesound : WAV_FIRE, pan(x));
			}
		}
		else
		{
			if (((id == wMagic && linkedItem && linked_itm.type == itype_book &&
				(linked_itm.flags & item_flag1))) && Lwpns.idCount(wFire) < 2)
			{
				Lwpns.add(new weapon(x, y - fakez, z, wFire, 2, zc_max(1, linked_itm.misc4) * game->get_hero_dmgmult(), 0, linkedItem, -1, false, 0, 1));
				if (FFCore.getQuestHeaderInfo(vZelda) < 0x255)
				{
					sfx(WAV_FIRE, pan(x));
				}
				else sfx(linked_itm.usesound > 0 ? linked_itm.usesound : WAV_FIRE, pan(x));
			}
		}
		break;
	}
...
bool weapon::hit(sprite *s)
{
	...
	if(!get_qr(qr_OLD_BOMB_HITBOXES) && (id == wBomb || id == wSBomb || id == ewBomb || id == ewSBomb))
	{
		...
		auto radius = 0;
		if (valid_item_id(parentitem))
			radius = itemsbuf.get(parentitem).misc7;
		if(radius < 1)
		{
		...
		{
			return (distance(x+8,y+8-fakez,s->x+s->hit_width/2,s->y+s->hit_height/2) <= radius);
		}
	}
    return sprite::hit(s);
}
...
bool weapon::hit(int32_t tx2,int32_t ty2,int32_t tz2,int32_t txsz2,int32_t tysz2,int32_t tzsz2)
{
	...
	if(!get_qr(qr_OLD_BOMB_HITBOXES) && (id == wBomb || id == wSBomb || id == ewBomb || id == ewSBomb))
	{
		...
		{
			return (distance(x+8,y+8-fakez,tx2+txsz2/2,ty2+tysz2/2) <= radius);
		}
	}
	...
	switch(id)
	{
		...
		case wBrang: case ewBrang:
		{
			if (!suspt)
			{
				...
				if((id == wBrang && (!valid_parent || !(prnt_itm.flags & item_flag1)))
				...
				if(dead>0)
				{
					if(prnt_itm.flags & item_flag1)
					{
						if ( do_animation ) 
						{
							tile=o_tile+(frames*(prnt_itm.flags & item_flag2)?8:1);
						}
					}
					else
					{
	...
	switch(id)
	{
		...
		case wHammer:
			if(Hero.is_hitflickerframe() ||
					Hero.invis_timer || hero_scr->flags3&fINVISHERO)
				return;
...
void weapon::draw_hitbox()
{
	...
	if(!get_qr(qr_OLD_BOMB_HITBOXES) && (id == wBomb || id == wSBomb || id == ewBomb || id == ewSBomb))
	{
		...
		{
			al_draw_circle(x+8+0.5, y+8-fakez+playing_field_offset+0.5, radius, hitboxColor(info_opacity), 1);
		}
		end_info_bmp();
#endif
		return;
