...
static optional<int> get_hit_mirror(int x, int y, int ctype, dword refl_flags)
{
	auto layer_count = get_qr(qr_MIRROR_PRISM_LAYERS) ? 7 : 1;
	for(int q = 0; q < layer_count; ++q)
	{
		int cid = MAPCOMBO2(q-1,x,y);
		auto const& cmb = combobuf[cid];
		if(cmb.type != ctype) continue;
		if(!cmb.attributes[0] || (cmb.attributes[0] & refl_flags))
			return cid;
	}
	return nullopt;
}
bool weapon::do_mirror() // returns true if animate needs to early-break
{
	...
	auto make_mirror_w = [&](bool dupe)
		{
			weapon *w = nullptr;
			switch(id)
			{
				case wBeam: case wRefBeam: case ewSword: case wMagic: case wRefMagic:
				{
					if(dupe)
					{
						w = new weapon(*this);
						if (!Lwpns.add(w)) w = nullptr;
					}
					else w = this;
					break;
				}
	...
	if(get_hit_mirror(checkx, checky, cMIRROR, refl_flag))
	{
		weapon* w = make_mirror_w(false);
		if(!w) return true;
		...
		w->x=TRUNCATE_TILE(posx)+check_x_ofs;
		w->y=TRUNCATE_TILE(posy)+check_y_ofs;
	}
	if(get_hit_mirror(checkx, checky, cMIRRORSLASH, refl_flag))
	{
		weapon* w = make_mirror_w(false);
		if(!w) return true;
	...
	if(get_hit_mirror(checkx, checky, cMIRRORBACKSLASH, refl_flag))
	{
		weapon* w = make_mirror_w(false);
		if(!w) return true;
	...
	if(can_duplicate && get_hit_mirror(checkx, checky, cMAGICPRISM, refl_flag))
	{
	...
	if(can_duplicate && get_hit_mirror(checkx, checky, cMAGICPRISM4, refl_flag))
	{
		int32_t newx=TRUNCATE_TILE(posx)+check_x_ofs;
		int32_t newy=TRUNCATE_TILE(posy)+check_y_ofs;
	...
	if(auto newmirror = get_hit_mirror(checkx, checky, cMIRRORNEW, refl_flag))
	{
