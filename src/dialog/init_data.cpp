...
arr[ind] = Checkbox(checked = local_zinit.litems[ind+levelsOffset]&flag, \
	...
	return Row(
		...
		LEVEL_CBOX(l_custom1, liCUSTOM1),
		LEVEL_CBOX(l_custom2, liCUSTOM2),
		LEVEL_CBOX(l_custom3, liCUSTOM3),
	...
	switch(msg.message)
	{
		case message::LEVEL:
		{
			for(int32_t q = 0; q < 10; ++q)
			{
				...
				l_custom1[q]->setChecked(local_zinit.litems[q+levelsOffset] & liCUSTOM1);
				l_custom2[q]->setChecked(local_zinit.litems[q+levelsOffset] & liCUSTOM2);
				l_custom3[q]->setChecked(local_zinit.litems[q+levelsOffset] & liCUSTOM3);
