...
uint64_t version_key_of(zquestheader const& h)
{
	...
	if (h.new_version_id_main > 0)
	{
		...
		return (1ull << 62)
			| ((uint64_t)(h.new_version_id_main & 0xFF) << 54)
			| ((uint64_t)(h.new_version_id_second & 0xFF) << 46)
			| ((uint64_t)(h.new_version_id_third & 0xFF) << 38)
			| (date_min & ((1ull << 38) - 1));
	}
...
void bake_icon(Entry& e, int32_t t)
{
	...
	for (int i = 0; i < 256; i++)
	{
		...
		any = true;
		uint8_t* px = &rgba[i * 4];
		// colordata components are 6-bit (0-63); upscale like every other
		// palette use (see render_a5_color).
		px[0] = _rgb_scale_6[pal[v * 3 + 0] & 63];
		px[1] = _rgb_scale_6[pal[v * 3 + 1] & 63];
		px[2] = _rgb_scale_6[pal[v * 3 + 2] & 63];
		px[3] = 255;
	}
