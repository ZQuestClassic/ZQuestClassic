...
std::shared_ptr<GUI::Widget> FFCDialog::view()
{
	...
	window = Window(
		title = fmt::format("FFC Editor ({})", ffind+1),
					...
					TabPanel(ptr = &ffctabs[1],
						...
						TabRef(name = "Script", Column(
							Button(
								...
								onPressFunc = [&]()
								{
									ScriptDataDialog(fmt::format("FFC #{} Script Setup", ffind+1),
										ffc.scrconfig, list_ffcscript, ffscripts).show();
								}
							),
							Rows<2>(
...
bool FFCDialog::handleMessage(const GUI::DialogMessage<message>& msg)
{
	...
	{
		case message::OK:
			if(ffc.scrconfig.script && !ffc.data)
			{
