...
int32_t _jwin_do_menu(MENU *menu, MENU_INFO *parent, int32_t bar, int32_t x, int32_t y, int32_t repos, int32_t *dret, int32_t minw, int32_t minh)
{
    ...
    do
    {
        ...
        else                                                    /* button not pressed */
        {
            ...
            {
                if(joy[joystick_index].stick[0].axis[1].d1)
                {
                    c = KEY_UP    << 8;
                    joy_on = TRUE;
                }
                
                if(joy[joystick_index].stick[0].axis[1].d2)
                {
                    c = KEY_DOWN  << 8;
                    joy_on = TRUE;
                }
                
                if(joy[joystick_index].stick[0].axis[0].d1)
                {
                    c = KEY_LEFT  << 8;
                    joy_on = TRUE;
                }
                
                if(joy[joystick_index].stick[0].axis[0].d2)
                {
                    c = KEY_RIGHT << 8;
                    joy_on = TRUE;
                }
                
                if(joy[joystick_index].button[0].b | joy[joystick_index].button[1].b)
                {
                    c = KEY_ENTER << 8;
                    joy_on = TRUE;
                }
