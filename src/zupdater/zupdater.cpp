...
static std::tuple<std::string, std::string> get_next_release()
{
	...
	// A response that parses but does not have the expected schema must not take the
	// process down: reading a missing or mistyped field throws, and nothing between
	// here and main catches it.
	try
	{
		auto data = json::parse(chunk.memory, nullptr, false);
		if (data.is_object())
		{
			if (auto tag = data.find("tagName"); tag != data.end() && tag->is_string())
				tag_name = tag->get<std::string>();

			if (auto assets = data.find("assets"); assets != data.end() && assets->is_array())
			{
				for (auto& asset_json : *assets)
				{
					if (!asset_json.is_object())
						continue;

					auto name = asset_json.find("name");
					auto url = asset_json.find("url");
					if (name == asset_json.end() || !name->is_string())
						continue;
					if (url == asset_json.end() || !url->is_string())
						continue;

					if (name->get<std::string>().find(platform) != std::string::npos)
					{
						asset_url = url->get<std::string>();
						break;
					}
				}
			}
	...
	{
		fprintf(stderr, "failed to read release json: %s\n", e.what());
		tag_name.clear();
		asset_url.clear();
	}
