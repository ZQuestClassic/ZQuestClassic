...
/*DrawBitmapEx
 * My apologies for the size of this function, but ag4 does not have a single functon that supports all of 
	these modes.
 * To simplify using it from a user perspective), I employed stacked flags, that when evaluated by the
	switch statement jump to the proper case value with	 instructions matching the desired user mode. -Z
