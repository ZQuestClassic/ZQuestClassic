...
ScopedQuestHeaderGlobals::ScopedQuestHeaderGlobals()
	...
	, held_midi_flags(midi_flags, midi_flags + MIDIFLAGS_SIZE)
...
ScopedQuestHeaderGlobals::~ScopedQuestHeaderGlobals()
{
	...
	memcpy(midi_flags, held_midi_flags.data(), held_midi_flags.size());
...
int32_t readrules(PACKFILE *f, zquestheader *Header)
{
	...
	// After this point, these QRs are for changes that were cherry-picked
		...
		if((Header->zelda_version < 0x192)||((Header->zelda_version == 0x192)&&(Header->build<154)))
		{
			for(int32_t j=0; j<MAPSCRS; j++)
			{
				...
				for(int32_t k=0; k<128; k++)
				{
					...
					word pos = TheMaps[scr].secretcombo[k];
					if(pos >= 176)
						pos = 0;
					TheMaps[scr].secretcset[k]=tcmbcset2(i, pos);
					TheMaps[scr].secretflag[k]=tcmbflag2(i, pos);
					TheMaps[scr].secretcombo[k]=tcmbdat2(i, j, pos);
				}
			}
		}
	}
...
static int32_t _lq_int(const char *filename, zquestheader *Header, miscQdata *Misc, zctune *tunes, bool show_progress, byte *skip_flags, byte printmetadata, bool stream_decode)
{
    ...
    if(tempheader.zelda_version>=0x193)
    {
        ...
        for (dword id : {ID_RULES, ID_STRINGS, ID_MISC, ID_TILES, ID_COMBOS,
            ...
            ID_CHEATS, ID_ITEMDROPSETS, ID_FAVORITES})
        {
