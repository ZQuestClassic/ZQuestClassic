...
void FFScript::do_BITMAP_drawing_command(const int script_command)
{
    int j = script_drawing_commands.GetNext();
    ...
    script_drawing_commands[j][0] = script_command;
    script_drawing_commands[j][18] = ri->bitmapref; //the current bitnap ID
...
case DMAPDATASETMUSICV: //command, string to load a music file
	...
	{
		FFCore.do_BITMAP_drawing_command(scommand);
		break;
