...
ZCMUSIC * zcmusic_load_file(const char *filename)
{
	...
	if((strcasecmp(ext,"ogg")==0) && (libflags & ZCMF_OGG))
	{
	...
	if((strcasecmp(ext,"mp3")==0) && (libflags & ZCMF_MP3))
	{
		ALSTREAMFILE *p = load_alstream_file(filename);
	...
	if(((strcasecmp(ext,"it")==0) || (strcasecmp(ext,"s3m")==0) || (strcasecmp(ext,"mod")==0)|| (strcasecmp(ext,"xm")==0)) && libflags & ZCMF_DUH)
	{
		ALSTREAMFILE *p = load_alstream_file(filename);
	...
	if((libflags & ZCMF_GME))
	{
		if((strcasecmp(ext,"spc")==0) || (strcasecmp(ext,"gbs")==0) || (strcasecmp(ext,"vgm")==0)|| (strcasecmp(ext,"gym")==0)|| (strcasecmp(ext,"nsf")==0))
		{
