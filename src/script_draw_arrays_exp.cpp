...
//only if the player is messing with their pointers...
    ZScriptArray& InvalidZSError(const long ptr)
    {
        Z_scripterrlog("Invalid pointer (%i) passed to array (don't change the values of your array pointers)\n", ptr);
        return INVALIDARRAY;
    }
    ZScriptArray& getZSArray(const long ptr)
    {
        if(ptr <= 0)
            return InvalidZSError(ptr);
...
void do_fasttilesr(BITMAP *bmp, int *sdci, int xoffset, int yoffset)
{
    ...
    Z_scripterrlog("PutPixels array ID is%d: \n",sdci[2]/10000);
	ZScriptArray &a = getZSArray(sdci[2]/10000);
    int sz = a.Size();
	Z_scripterrlog("PutPixels array size is%d: \n",sz);
    ...
    {	
	    points[q] = (FFCore.getElement(sdci[2]/10000, q))/10000;
		//Z_scripterrlog("PutPixels array index %d is %d: \n",q, points[q]);
	Z_scripterrlog("drawtiles() points[%d] is: %d\n", q, points[q]);
    }
...
void do_fastcombosr(BITMAP *bmp, int *sdci, int xoffset, int yoffset)
{
    ...
    int sz = FFCore.getSize(sdci[2]/10000);
	Z_scripterrlog("DrawCombos() array size is: %d\n",sz);
	//Z_scripterrlog("PutPixels array size is%d: \n",sz);
    int points[214745];// = script_drawing_commands[i].GetPtr();
	memset(points,0,sizeof(points));
