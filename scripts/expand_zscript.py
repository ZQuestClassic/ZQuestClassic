...
parser.add_argument(
    '--name', help='The name to reserve for (will be capitalized for you)'
)
parser.add_argument(
    '--count', type=int, help='How many opcodes or registers to reserve'
)
parser.add_argument(
    '--listfile',
    help='''Specify a file to read names from each line, instead of a name/count.
...
exclgroup.add_argument(
    '--unopcode', '-u', action='store_true', help='Add Unary ZASM opcodes'
)
exclgroup.add_argument(
    '--binopcode', '-b', action='store_true', help='Add Binary ZASM opcodes'
)
exclgroup.add_argument(
    '--ternopcode', '-t', action='store_true', help='Add Ternary ZASM opcodes'
)
...
opcodetype = (
    3
    if args.ternopcode
    else (2 if args.binopcode else (1 if args.unopcode else (0 if args.opcode else -1)))
)

if not (args.reg or opcodetype > -1):
    print(
        "Specify either '--reg' or '--opcode'/'--unopcode'/'--binopcode' to choose a mode!",
        file=sys.stderr,
    )
    exit(1)
        ...
        [
            (
                ...
                if pat_spec.match(x)
                    ...
                    x.upper()
                    if ind < 1 or pat_wordbreak.match(s[ind - 1])
                    else x.lower()
                )
            )
...
if args.listfile:
    if args.name or args.count:
        print(
            "Cannot specify '--listfile' alongside '--name' or '--count'!",
            file=sys.stderr,
        )
        exit(1)
    ...
    with open(args.listfile, 'r') as file:
        ...
        if opcodetype < 0:
            ...
            oplist_allcaps = [
                re.match(oppat_1, x).groups()[0]
                for x in tmplines
                if re.match(oppat_1, x)
            ]
            oplist_propcase = [
                re.match(oppat_2, x).groups()[0]
                for x in tmplines
                if re.match(oppat_2, x)
            ]
    ...
    if args.reg:
        ...
        while lines[index].strip() != 'default:':
            m = re.match(
                f'case RESRVD_VAR_{args.name}_?([0-9]+): return "RESRVD_VAR_{args.name}_?[0-9]+";',
                lines[index].strip(),
            )
            if m:
        ...
        while index < len(lines):
            m = re.match(
                f'string OReserved{name_prop}_?([0-9]+)::toString\\(\\) const',
                lines[index].strip(),
            )
            if m:
                val = int(m.groups()[0])
...
if args.reg:
    ...
    for q in range(args.count):
        ...
        match opcodetype:
            case 3:
                newlines.append(
                    f'\treturn "{oplist_allcaps[q]} " + getFirstArgument()->toString() + "," + getSecondArgument()->toString() + "," + getThirdArgument()->toString();\n'
                )
            case 2:
                newlines.append(
                    f'\treturn "{oplist_allcaps[q]} " + getFirstArgument()->toString() + "," + getSecondArgument()->toString();\n'
                )
            case 1:
                newlines.append(
                    f'\treturn "{oplist_allcaps[q]} " + getArgument()->toString();\n'
                )
            case _:
    ...
    for op in oplist_propcase:
        match opcodetype:
            case 3:
                ...
                newlines.append(
                    f'\t\t{op}(Argument *A, Argument* B, Argument* C) : TernaryOpcode(A,B,C) {{}}\n'
                )
                newlines.append(f'\t\tstd::string toString() const;\n')
                newlines.append(f'\t\tOpcode* clone() const\n')
                newlines.append(f'\t\t{{\n')
                newlines.append(
                    f'\t\t\treturn new {op}(a->clone(),b->clone(),c->clone());\n'
                )
                newlines.append(f'\t\t}}\n')
                newlines.append(f'\t}};\n')
                newlines.append(f'\n')
            case 2:
                ...
                newlines.append(f'\tpublic:\n')
                newlines.append(
                    f'\t\t{op}(Argument *A, Argument* B) : BinaryOpcode(A,B) {{}}\n'
                )
                newlines.append(f'\t\tstd::string toString() const;\n')
                newlines.append(f'\t\tOpcode* clone() const\n')
                newlines.append(f'\t\t{{\n')
