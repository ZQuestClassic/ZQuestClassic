	...
	for i, part in enumerate(replay_parts):
				...
				if last_hero_position:
					...
					for j, step in enumerate(part.steps):
						...
						if step.type == 'C' and step.data.startswith('scr='):
							part.steps.insert(j + 1, ReplayStep('C', 0, f'h {last_hero_position}'))
							inserted = True
							break

					if not inserted:
						raise Exception(f'[{output_replay.name}] could not find where to insert hero position')
		...
		with output_replay.open('w', newline='\n') as f:
			...
			if most_recent_key_step:
				f.write(' '.join([most_recent_key_step.type, '0', most_recent_key_step.data]))
				f.write('\n')
