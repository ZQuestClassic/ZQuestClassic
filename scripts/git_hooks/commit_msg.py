...
match = re.match(r'(\w+)\((\w+)\)[!<]?: (.+)', first_line) or re.match(
    r'(\w+)[!<]?:( )(.+)', first_line
)
if not match:
    add_log(
    ...
    add_log(f'valid types (the first word of the commit) are:', ', '.join(valid_types))
    add_log(
        f'valid scopes (the optional text in parentheses) are:', ', '.join(valid_scopes)
    )
    add_log(
        'A "!" may be added before the ":" to remove the commit from the changelog.'
    )
    add_log(
        'A "<" may be added before the ":" to squash the commit into the previous in the changelog.'
    )
    add_log()
    is_valid = False
else:
