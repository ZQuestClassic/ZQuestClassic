...
generic script garbage_collection
{
	...
	void run()
	{
		...
		// 	// check("count (c)", count, 1); // It still exists.
		// 	// The autorelease pool is drained when the script yields.
		// 	// yield();
		// 	// check("count (d)", count, 0);
		...
		{
			Person a = new Person();
			check("RefCount(a) (1)", RefCount(a), 1L);
			// a = a;
		...
		// printf("=== Test %d - untyped return === \n", ++tests);
		// {
		// 	untyped person = maybeCreatePersonAsUntyped(true);
		// 	yield();
		// 	check("RefCount(person)", RefCount(person), 1L);
		// 	check("(1) count", count, 1);

		// 	person = maybeCreatePersonAsUntyped(false); // TODO !
		// 	check("RefCount(person)", RefCount(person), -1L);
		// 	check("(2) count", count, 0);

		// 	person = maybeCreatePersonAsUntyped(true);
		// 	check("(3) count", count, 1);
		// }
		// check("(4) count", count, 0);

		// // TODO ! maybeUsePersonAsUntyped
		...
		// 	check("RefCount(a)", RefCount(a), 2L);
		// 	a = NULL;
		// 	check("RefCount(a)", RefCount(b), 1L);
		// 	b = NULL;
		// 	checkCountWithGC(0);
		// }
		// check("count", count, 0);
		...
		// 		check("RefCount(retainer)", RefCount(retainer), 2L);
	...
	untyped maybeCreatePersonAsUntyped(bool returnPerson)
	{
		if (!returnPerson) return 1337;
		return new Person();
	}

	untyped maybeUsePersonAsUntyped(untyped maybePerson, bool makeNew)
	{
		if (RefCount(maybePerson) != -1)
		{

		}

		if (!makeNew) return maybePerson;
		return new Person();
	}
