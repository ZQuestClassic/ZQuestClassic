...
async def update_views():
	...
	for view in views:
		...
		if type == 'results':
			...
			test_results_path = root_dir/'.tmp/test_results'/test_results_name/'test_results.json'
...
async def handle_compare_command(client: Client, params):
	def create_proc(program: str, args: List[str]):
		return asyncio.create_subprocess_exec(program, *args, cwd=root_dir,
	...
	if await p.wait() != 0:
		...
		await update_view(view, {'status': f'[FAILED] downloading baseline build {tag}\n' + stderr})
		return
	...
	baseline_test_results_dir = root_dir/'.tmp/baseline_test_results'/f'{test_results_name}-baseline'
	...
	args = [
		...
		'--test_results_folder', str(baseline_test_results_dir),
		'--retries=2',
	...
	if await p.wait() != 0:
		stderr = (await p.stderr.read()).decode('utf-8')
		await update_view(view, {'status': f'[FAILED] running baseline replays\n{stderr}'})
		return
	...
	args = [
		...
		'--local', baseline_test_results_dir,
	...
	async for msg in ws:
		if msg.type == aiohttp.WSMsgType.TEXT:
			if msg.data == 'close':
				...
				if co:
					asyncio.run_coroutine_threadsafe(co(client, data['params']), asyncio.get_running_loop())
